...
class Reports(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        ...
        submited = Log.objects.filter(
            user_id__in=users,
            action="submit",
            resource="goals",
            context__contains={"goals_id": goal.id},
        ).values_list("user_id")
...
class InsideView(LoginRequiredMixin, LogMixin, generic.ListView):
    ...
    def get_queryset(self):
        ...
        if has_subject_permissions(self.request.user, goal.topic.subject):
            self.students = User.objects.filter(
                subject_student=goal.topic.subject
    ...
    def post(self, request, *args, **kwargs):
        ...
        if has_subject_permissions(request.user, goal.topic.subject):
            ...
            if not user is None:
                self.object_list = MyGoals.objects.filter(
                    user__email=user, item__goal=goal
                )
            else:
                self.object_list = MyGoals.objects.filter(
                    user=self.request.user, item__goal=goal
                )
        else:
            self.object_list = MyGoals.objects.filter(
                user=self.request.user, item__goal=goal
...
class NewWindowSubmit(LoginRequiredMixin, LogMixin, generic.edit.CreateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        my_goals_formset = MyGoalsFormset(
            initial=[
                {"item": x.id, "value": x.ref_value} for x in goals.item_goal.all()
            ]
        )
        ...
        super(NewWindowSubmit, self).createLog(
            self.request.user,
            self.log_component,
            self.log_action,
            self.log_resource,
        ...
        return self.render_to_response(
            self.get_context_data(my_goals_formset=my_goals_formset)
        )
    ...
    def form_valid(self, my_goals_formset):
        ...
        if not dificulties is None and dificulties.strip():
            ...
            message = (
                _("#Dificulty(ies) found in %s") % (str(goals))
                + ":<p>"
                + dificulties
                + "</p>"
            )
...
class SubmitView(LoginRequiredMixin, LogMixin, generic.edit.CreateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        super(SubmitView, self).createLog(
            self.request.user,
            self.log_component,
            self.log_action,
            self.log_resource,
...
class UpdateSubmit(LoginRequiredMixin, LogMixin, generic.UpdateView):
    ...
    def get_success_url(self):
        ...
        super(UpdateSubmit, self).createLog(
            self.request.user,
            self.log_component,
            self.log_action,
            self.log_resource,
...
class CreateView(LoginRequiredMixin, LogMixin, generic.edit.CreateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return self.render_to_response(
            self.get_context_data(form=form, goalitems_form=goalitems_form,)
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid() and goalitems_form.is_valid():
            return self.form_valid(form, goalitems_form)
        else:
            return self.form_invalid(form, goalitems_form)
    ...
    def form_invalid(self, form, goalitems_form):
        return self.render_to_response(
            self.get_context_data(form=form, goalitems_form=goalitems_form,)
        )

    def form_valid(self, form, goalitems_form):
        ...
        pendency.begin_date = self.object.init_date
        pendency.end_date = self.object.limit_submission_date
        pendency.limit_date = self.object.limit_submission_date
    ...
    def get_success_url(self):
        messages.success(
            self.request,
            _("The Goals specification for the topic %s was realized successfully!")
        ...
        if has_subject_permissions(self.request.user, self.object.topic.subject):
            ...
            success_url = reverse_lazy(
                "goals:submit", kwargs={"slug": self.object.slug}
            )
            ...
            if self.object.show_window:
                ...
                self.request.session["resources"]["new_page_url"] = reverse(
                    "goals:window_submit", kwargs={"slug": self.object.slug}
                )
...
class StatisticsView(LoginRequiredMixin, LogMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        vis_ou = Log.objects.filter(
            ...
            datetime__range=(start_date, end_date + datetime.timedelta(minutes=1)),
        ...
        for log_al in vis_ou.order_by("datetime"):
            data_history.append(
                [
                    str(alunos.get(email=log_al.user_email)),
                    ", ".join(
                        [
                            str(x)
                            for x in goal.topic.subject.group_subject.filter(
