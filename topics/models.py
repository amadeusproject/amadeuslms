...
class Resource(KnowsChild):
    ...
    visible = models.BooleanField(_('Visible'), default=True)
    order = models.PositiveSmallIntegerField(_('Order'), null=True)
    topic = models.ForeignKey(Topic, verbose_name=_('Topic'), related_name="resource_topic",
                              null=True, on_delete=models.CASCADE)
    students = models.ManyToManyField(User, verbose_name=_('Students'),
                                      related_name='resource_students', blank=True)
    groups = models.ManyToManyField(StudentsGroup, verbose_name=_('Groups'),
                                    related_name='resource_groups', blank=True)
    tags = models.ManyToManyField(Tag, verbose_name=_('Markers'), related_name='resource_tags',
                                  blank=True)
    create_date = models.DateTimeField(_('Create Date'), auto_now_add=True)
    last_update = models.DateTimeField(_('Last Update'), auto_now=True)
