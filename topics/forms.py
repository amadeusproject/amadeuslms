...
class TopicForm(forms.ModelForm):
	...
	def clean_name(self):
		...
		if self.instance.id:
			same_name = self.subject.topic_subject.filter(name__iexact = name).exclude(id = self.instance.id).count()
		else:
			same_name = self.subject.topic_subject.filter(name__iexact = name).count()
