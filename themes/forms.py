...
class BasicElemetsForm(forms.ModelForm):
        ...
        if image:
            if hasattr(image, '_size'):
                if image._size > self.MAX_UPLOAD_SIZE:
                    self._errors['favicon'] = [
                        _("The image is too large. It should have less than 2MB.")]
    ...
    def clean_high_contrast_logo(self):
        ...
        if image:
            if hasattr(image, '_size'):
                if image._size > self.MAX_UPLOAD_SIZE:
                    self._errors['high_contrast_logo'] = [
                        _("The image is too large. It should have less than 2MB.")]
    ...
    class Meta:
        model = Themes
        fields = ['title', 'favicon', 'small_logo', 'large_logo', 'high_contrast_logo',
                  'footer_note']
        widgets = {
            'favicon': forms.FileInput(attrs={'accept': 'image/*'}),
            'small_logo': forms.FileInput(attrs={'accept': 'image/*'}),
            'large_logo': forms.FileInput(attrs={'accept': 'image/*'}),
            'high_contrast_logo': forms.FileInput(attrs={'accept': 'image/*'}),
        }
