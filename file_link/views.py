...
class DownloadFile(LoginRequiredMixin, LogMixin, generic.DetailView):
    ...
    def render_to_response(self, context, **response_kwargs):
        ...
        super(DownloadFile, self).createLog(
            self.request.user,
            self.log_component,
            self.log_action,
            self.log_resource,
...
class CreateView(LoginRequiredMixin, LogMixin, generic.edit.CreateView):
    ...
    def get_success_url(self):
        messages.success(
            self.request,
            _(
                'The File Link "%s" was added to the Topic "%s" of the virtual environment "%s" successfully!'
            )
            % (
                self.object.name,
...
class UpdateView(LoginRequiredMixin, LogMixin, generic.UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["resource"] = get_object_or_404(
            FileLink, slug=self.kwargs.get("slug", "")
        )
    ...
    def get_success_url(self):
        messages.success(
            self.request,
            _('The File Link "%s" was updated successfully!') % (self.object.name),
        )
...
class StatisticsView(LoginRequiredMixin, LogMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        vis_ou = Log.objects.filter(
            context__contains={"filelink_id": filelink.id},
            resource="filelink",
            action="view",
            ...
            data_history.append(
                [
                    ...
                    ", ".join(
                        [
                            str(x)
                            for x in filelink.topic.subject.group_subject.filter(
...
class SendMessage(LoginRequiredMixin, LogMixin, generic.edit.FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["filelink"] = get_object_or_404(
            FileLink, slug=self.kwargs.get("slug", "")
        )
        return context
