...
class ViewPDFFile(LoginRequiredMixin, generic.TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not has_subject_permissions(request.user,
                                       topic.subject) and not has_resource_permissions(request.user,
                                                                                       resource):
    ...
    def render_to_response(self, context, **response_kwargs):
        ...
        if pdf_file.show_window:
            response = HttpResponse(open(pdf_file.file.path, 'rb').read(),
...
class PDFFileCreateView(LoginRequiredMixin, generic.CreateView):
    ...
    def get_success_url(self):
        ...
        return reverse_lazy('subjects:topic_view', kwargs={'slug': self.object.topic.subject.slug,
...
class StatisticsView(LoginRequiredMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        vis_ou = Log.objects.filter(context__contains={'pdffile_id': pdf_file.id},
                                    resource="pdffile", action="view",
        ...
        for log_al in vis_ou.order_by("datetime"):
            data_history.append([str(alunos.get(email=log_al.user_email)),
                                 ", ".join([str(x) for x in
                                            pdf_file.topic.subject.group_subject.filter(
