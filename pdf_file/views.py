...
class ViewPDFFile(LoginRequiredMixin, LogMixin, generic.TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not has_subject_permissions(
            request.user, topic.subject
        ) and not has_resource_permissions(request.user, resource):
    ...
    def get_context_data(self, **kwargs):
        ...
        super(ViewPDFFile, self).createLog(
            self.request.user,
            self.log_component,
            self.log_action,
            self.log_resource,
    ...
    def render_to_response(self, context, **response_kwargs):
        ...
        if pdf_file.show_window:
            response = HttpResponse(
                open(pdf_file.file.path, "rb").read(), content_type="application/pdf"
            )
            return response
...
class PDFFileCreateView(LoginRequiredMixin, LogMixin, generic.CreateView):
    ...
    def form_valid(self, form, pendencies_form):
        ...
        super(PDFFileCreateView, self).createLog(
            self.request.user,
            self.log_component,
            self.log_action,
            self.log_resource,
    ...
    def get_success_url(self):
        messages.success(
            self.request,
            _(
                'The PDF File "%s" was added to the Topic "%s" of the virtual environment "%s" successfully!'
            )
            % (
                self.object.name,
        ...
        return reverse_lazy(
            "subjects:topic_view",
            kwargs={
                "slug": self.object.topic.subject.slug,
...
class UpdateView(LoginRequiredMixin, LogMixin, generic.UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["resource"] = get_object_or_404(
            PDFFile, slug=self.kwargs.get("slug", "")
        )
    ...
    def get_success_url(self):
        messages.success(
            self.request,
            _('The PDF File "%s" was updated successfully!') % (self.object.name),
        )
...
class StatisticsView(LoginRequiredMixin, LogMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        vis_ou = Log.objects.filter(
            context__contains={"pdffile_id": pdf_file.id},
            resource="pdffile",
            action="view",
            ...
            data_history.append(
                [
                    ...
                    ", ".join(
                        [
                            str(x)
                            for x in pdf_file.topic.subject.group_subject.filter(
...
class SendMessage(LoginRequiredMixin, LogMixin, generic.edit.FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["pdf_file"] = get_object_or_404(
            PDFFile, slug=self.kwargs.get("slug", "")
        )
        return context
