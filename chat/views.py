...
class GeneralIndex(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        ...
        conversations = Conversation.objects \
            .extra(select={
            "most_recent":
                "select create_date from chat_talkmessages where chat_talkmessages.talk_id = "
                "chat_conversation.id order by create_date DESC LIMIT 1"}) \
            .filter((Q(user_one=user) | Q(user_two=user))).order_by('-most_recent')
...
class GeneralParticipants(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        ...
        users = User.objects.filter(
            Q(username__icontains=search) | Q(last_name__icontains=search) | Q(
                social_name__icontains=search) | Q(email__icontains=search)).distinct().order_by(
            'social_name', 'username').exclude(email=user.email)
...
class SubjectParticipants(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        ...
        users = User.objects.filter((Q(username__icontains=search) | Q(
            last_name__icontains=search) | Q(social_name__icontains=search) | Q(
            email__icontains=search)) & (Q(is_staff=True) | Q(subject_student__id=sub) | Q(
            professors__id=sub) | Q(coordinators__subject_category__id=sub))).distinct().order_by(
            'social_name', 'username').exclude(email=user.email)
...
class SubjectView(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        ...
        conversations = Conversation.objects.extra(select={
            "most_recent": "select create_date from chat_talkmessages where chat_talkmessages.talk_id = chat_conversation.id order by create_date DESC LIMIT 1"}).filter(
            (Q(user_one=user) & (Q(user_two__is_staff=True) |
                                 Q(user_two__subject_student=subject) | Q(
                        user_two__professors=subject) | Q(
                        user_two__coordinators__subject_category=subject))) |
            (Q(user_two=user) & (Q(user_one__is_staff=True) | Q(user_one__subject_student=subject) |
                                 Q(user_one__professors=subject) | Q(
                        user_one__coordinators__subject_category=subject)))).distinct().order_by(
            '-most_recent')
...
class GetTalk(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        ...
        talks = Conversation.objects.filter((Q(user_one=user) & Q(user_two__email=user_email)) | (
    ...
    def get_context_data(self, **kwargs):
        ...
        context['form_url'] = reverse_lazy('chat:create', args=(),
                                           kwargs={'email': self.kwargs.get('email', ''),
                                                   'talk_id': self.talk_id,
                                                   'space': self.request.GET.get('space', '0'),
                                                   'space_type': self.request.GET.get('space_type',
                                                                                      'general')})
...
class SendMessage(LoginRequiredMixin, generic.edit.CreateView):
    ...
    def form_valid(self, form):
        ...
        notification = {
            ...
            "complete": render_to_string("chat/_message.html", {"talk_msg": self.object},
                                         self.request),
        ...
        send_chat_push_notification(user, self.object)
    ...
    def get_context_data(self, **kwargs):
        ...
        context['form_url'] = reverse_lazy('chat:create', args=(),
                                           kwargs={'email': self.kwargs.get('email', ''),
                                                   'talk_id': self.kwargs.get('talk_id', '-1'),
                                                   'space': self.kwargs.get('space', '0'),
                                                   'space_type': self.kwargs.get('space_type',
                                                                                 'general')})
    ...
    def get_success_url(self):
        ...
        self.kwargs.get('space_type', 'general'), self.kwargs.get('email', ''),))
...
def render_message(request, talk_msg, talk_id, space, space_type, email):
    ...
    form_url = reverse_lazy('chat:create', args=(),
                            kwargs={'email': email, 'talk_id': talk_id, 'space': space,
    ...
    return JsonResponse(
        {'view': html, 'new_id': msg.id, 'talk_id': msg.talk.id, 'new_form_url': form_url,
...
def load_messages(request, talk):
    ...
    if not favorites:
        ...
        if mines:
            messages = TalkMessages.objects.filter(talk__id=talk,
                                                   ...
                                                   chat_favorites_message__user=user, user=user)
        else:
            messages = TalkMessages.objects.filter(talk__id=talk,
                                                   chat_favorites_message__isnull=False,
    ...
    return JsonResponse(
        {"messages": response, "count": messages.count(), "num_pages": paginator.num_pages,
