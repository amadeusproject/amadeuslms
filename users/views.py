...
class SupportView(
    ...
    form_class = SelectSupport
    success_url = reverse_lazy("users:support_select")

    def get_initial(self):
        user = User.objects.filter(is_support=True).first()

        initial = {}

        initial["support"] = user

        return initial

    def form_valid(self, form):
        selected = form.cleaned_data["support"]

        User.objects.all().update(is_support=False)

        selected.is_support = True
        selected.save()

        messages.success(self.request, _("Support member designed successfully."))

        return super(SupportView, self).form_valid(form)

    def get_context_data(self, **kwargs):
        context = super(SupportView, self).get_context_data(**kwargs)
        context["title"] = _("Designate Support")
