...
# ================ ADMIN =======================
class UsersListView(braces_mixins.LoginRequiredMixin, braces_mixins.StaffuserRequiredMixin,
                    generic.ListView):
    ...
    def get_queryset(self):
        users = User.objects.all().order_by('social_name', 'username').exclude(
...
class SearchView(braces_mixins.LoginRequiredMixin, braces_mixins.StaffuserRequiredMixin,
                 generic.ListView):
    login_url = reverse_lazy("users:login")
    redirect_field_name = 'next'
    ...
    def get_queryset(self):
        ...
        users = User.objects.filter(
            ...
            'social_name', 'username').exclude(email=self.request.user.email)
...
class DeleteView(braces_mixins.LoginRequiredMixin, LogMixin, generic.DeleteView):
    ...
    def render_to_response(self, context, **response_kwargs):
        ...
        return self.response_class(request=self.request, template=template, context=context,
            ...
            if users.exists():
                for user in users:
                    ...
                    c = {
                        ...
                        'email': user.email,
                        'domain': request.build_absolute_uri(reverse("users:reset_password_confirm",
                                                                     kwargs={'uidb64': uid,
                                                                             'token': token})),
                        # or your domain
                        'site_name': 'Amadeus',
                    ...
                    if mailsender.hostname == "example.com":
                        ...
                        backend = EmailBackend(
                            host=mailsender.hostname, port=mailsender.port,
                            username=mailsender.username,
                            password=mailsender.password, use_tls=tls
                        )

                        mail_msg = EmailMessage(subject=subject, body=email, to=[user.email],
                                                connection=backend)
                ...
                messages.success(request, _(
                    "Soon you'll receive an email with instructions to set your new password. If you don't receive it in 24 hours, please check your spam box."))
