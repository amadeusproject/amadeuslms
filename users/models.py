...
class User(AbstractBaseUser, PermissionsMixin):
    email = models.EmailField(_('Mail'), unique=True, validators=[
        validators.RegexValidator(
            re.compile('^[\w.@+-]+$'),
            _(
                'Type a valid email. This fields should only contain letters, numbers and the characteres: @/./+/-/_ .')
            , 'invalid'
        )
    ...
    show_email = models.IntegerField(_('Show email?'), null=True, blank=True, choices=(
        (1, _('Allow everyone to see my address')), (2, _('Only classmates can see my address')),
        (3, _('Nobody can see my address'))), default=1)
...
# Log models
class UserWasCreatedLog(Log):
    pass


class UpdateUserLog(Log):
    pass


class DeleteUserLog(Log):
    pass


class RemoveAccountLog(Log):
    pass


class UserAccessLog(Log):
    pass


class UserLogoutLog(Log):
    pass
