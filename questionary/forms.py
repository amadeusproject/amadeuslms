...
class SpecificationForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["categories"].queryset = Tag.objects.filter(
            question_categories__subject__id=self.subject
        ).distinct()
    ...
    def clean(self):
        ...
        if categories and (not n_questions or n_questions == ""):
            self.add_error(
                "n_questions",
                _("You must provide the number of questions for the combination."),
            )
        ...
        if n_questions and n_questions == "0":
            self.add_error(
                "n_questions",
                _(
                    "There is no questions for this combination of categories. Please try another combination."
                ),
            )
    ...
    def clean(self):
        ...
        if not n_questions:
            raise forms.ValidationError(
                _("It's necessary to enter at least one question specification.")
            )
...
InlineSpecificationFormset = inlineformset_factory(
    Questionary,
    Specification,
