...
class SubjectNotifications(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        ...
        if has_subject_permissions(self.request.user, subject):
            ...
            notifications = Notification.objects.filter(user=self.students.first(),
                                                        task__resource__topic__subject=subject,
                                                        creation_date=datetime.now()).order_by(
                "task__limit_date", "task__end_date")
        else:
    ...
    def post(self, request, *args, **kwargs):
        ...
        if has_subject_permissions(request.user, subject):
            ...
            if user is not None:
                ...
                self.object_list = Notification.objects.filter(user=self.request.user,
                                                               task__resource__topic__subject=subject,
                                                               creation_date=datetime.now()).order_by(
                    "task__limit_date", "task__end_date")
        else:
    ...
    def get_queryset(self):
        ...
        if cat:
            notifications = Notification.objects.filter(user=self.request.user,
                                                        ...
                                                        task__resource__topic__subject__category__slug=cat).values(
                'task__resource__topic__subject', 'task__resource__topic__subject__name').annotate(
                total=Count('task__resource__topic__subject'))
        else:
            notifications = Notification.objects.filter(user=self.request.user,
                                                        creation_date=datetime.now()).values(
                'task__resource__topic__subject', 'task__resource__topic__subject__name').annotate(
                total=Count('task__resource__topic__subject'))
...
class AjaxNotifications(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        ...
        notifications = Notification.objects.filter(user=self.request.user,
                                                    task__resource__topic__subject__id=subject_id,
                                                    creation_date=datetime.now()).order_by(
            "task__limit_date", "task__end_date")
...
class AjaxHistory(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        ...
        notifications = Notification.objects.filter(user=self.request.user,
                                                    task__resource__topic__subject__id=subject_id).order_by(
            *order)
...
def set_goal(request):
    if request.method == "POST" and request.is_ajax():
        ...
        if not notify_id:
            return JsonResponse(
                {'error': True, 'message': _('Could not identify the notification')})
        ...
        if meta < timezone.now():
            return JsonResponse({'error': True, 'message': _(
                "The goal date should be equal or after today's date")})

        if meta.date() > notification.task.resource.topic.subject.end_date:
            return JsonResponse({'error': True, 'message': _(
                "The goal date should be equal or before subject's date")})
        ...
        if notification.level == 2:
            message = _('Your new goal to realize the task %s is %s') % (
                str(notification), formats.date_format(meta, "SHORT_DATETIME_FORMAT"))
        else:
            message = _('Your goal to realize the task %s is %s') % (
                str(notification), formats.date_format(meta, "SHORT_DATETIME_FORMAT"))
