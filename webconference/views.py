...
class Conference(LoginRequiredMixin, LogMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        super(Conference, self).createLog(
            self.request.user,
            self.log_component,
            self.log_action,
            self.log_resource,
...
class CreateView(LoginRequiredMixin, LogMixin, generic.edit.CreateView):
    ...
    def get_success_url(self):
        ...
        success_url = reverse_lazy(
            "webconferences:view", kwargs={"slug": self.object.slug}
        )
...
class UpdateView(LoginRequiredMixin, LogMixin, generic.UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["resource"] = get_object_or_404(
            Webconference, slug=self.kwargs.get("slug", "")
        )
    ...
    def get_success_url(self):
        messages.success(
            self.request,
            _('The Web conference "%s" was updated successfully!') % (self.object.name),
        )
...
class ConferenceSettings(
    ...
    braces_mixins.StaffuserRequiredMixin,
    generic.UpdateView,
):
    login_url = reverse_lazy("users:login")
    redirect_field_name = "next"
...
class StatisticsView(LoginRequiredMixin, LogMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        vis_ou = Log.objects.filter(
            context__contains={"webconference_id": webconference.id},
            ...
            data_history.append(
                [
                    ...
                    ", ".join(
                        [
                            str(x)
                            for x in webconference.topic.subject.group_subject.filter(
        ...
        column_view, column_initwebconference, column_participate = (
            str(_("View")),
            str(_("Enter")),
            str(_("Participate")),
        )
        ...
        not_initwebconference = alunos.exclude(
            email__in=[
                log.user_email
                for log in vis_ou.filter(action="initwebconference").distinct(
                    "user_email"
                )
            ]
        )
        for alun in not_initwebconference:
            data_n_did.append(
                [
                    ...
                    column_initwebconference,
                    str(alun.email),
                ]
            )
            index += 1

        not_participate = alunos.exclude(
            email__in=[
                log.user_email
                for log in vis_ou.filter(action="participating").distinct("user_email")
            ]
        )
        ...
        c_initwebconference = (
            vis_ou.filter(action="initwebconference").distinct("user_email").count()
        )
        c_participate = (
            vis_ou.filter(action="participating").distinct("user_email").count()
        )
        ...
        re.append(
            [
                column_initwebconference,
                c_initwebconference,
                alunos.count() - c_initwebconference,
            ]
        )
...
class SendMessage(LoginRequiredMixin, LogMixin, generic.edit.FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["webconference"] = get_object_or_404(
            Webconference, slug=self.kwargs.get("slug", "")
        )
        return context
