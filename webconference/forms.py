...
class WebconferenceForm(forms.ModelForm):
    ...
    class Meta:
        model = Webconference
        fields = ['name', 'presentation', 'start', 'end', 'brief_description', 'all_students',
                  'students', 'groups', 'show_window', 'visible']
        labels = {
    ...
    def clean(self):
        ...
        for topic in topics:
            ...
            if same_name > 0:
                self.add_error('name',
                               _('This subject already has a web conference with this name'))

        if start and end:
            if not start == ValueError and not end == ValueError:
                if start > end:
                    self.add_error('start', _(
                        'This input should be filled with a date equal or before the End.'))
                    self.add_error('end', _(
                        'This input should be filled with a date equal or after the Start.'))
...
InlinePendenciesFormset = inlineformset_factory(Webconference, Pendencies,
                                                form=PendenciesLimitedForm, extra=1, max_num=3,
