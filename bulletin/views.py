...
class NewWindowView(LoginRequiredMixin, LogMixin, generic.DetailView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if has_subject_permissions(request.user, bulletin.topic.subject):
            ...
            if not difficulties is None and not difficulties == "":
                print(difficulties)
                message = (
                    _("#Difficulty(ies) found in %s") % (str(bulletin))
                    + ":<p>"
                    + difficulties
                    + "</p>"
                )
                ...
                messages.error(
                    self.request, message=_("You should inform some difficulty")
                )
                return self.render_to_response(context=self.get_context_data())
    ...
    def get_context_data(self, **kwargs):
        ...
        lista_metas = [
            {"description": geral.description, "desejada": geral.ref_value}
            for geral in itens_da_meta
        ]
        ...
        alcancadas, medias = read_excel_file(
            estudante, meta_geral, len(itens_da_meta), bulletin
        )
        maximos, medianas, resultados, titulos = read_excel_file_indicators(
            estudante, bulletin
        )
        ...
        context["student"] = self.request.POST.get(
            "selected_student", students.first().email
        )
...
class InsideView(LoginRequiredMixin, LogMixin, generic.DetailView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if has_subject_permissions(request.user, bulletin.topic.subject):
            ...
            if not difficulties is None and not difficulties == "":
                message = (
                    _("#Dificulty(ies) found in %s") % (str(bulletin))
                    + ":<p>"
                    + difficulties
                    + "</p>"
                )
...
class CreateView(LoginRequiredMixin, LogMixin, generic.edit.CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if existe_meta:
            meta_geral = Goals.objects.get(topic=topic)
            if (
                meta_geral.limit_submission_date.date()
        ...
        if existe_boletim:
            messages.error(
                request,
                _("The topic %s already has a Bulletin, so you can't create another.")
                % (topic),
            )
    ...
    def get_context_data(self, **kwargs):
        ...
        itens_da_meta = sorted(list(metas), key=lambda met: met.id)
        alunos = sorted(
            list(meta_geral.topic.subject.students.all()), key=lambda e: e.id
        )
    ...
    def get_success_url(self):
        ...
        if self.object.show_window:
            ...
            self.request.session["resources"]["new_page_url"] = reverse(
                "bulletin:window_view", kwargs={"slug": self.object.slug}
            )
...
def download_excel(request, file):
    filepath = os.path.join(
        "bulletin", os.path.join("sheets", os.path.join("xls", file + ".xls"))
    )
...
def read_excel_file_indicators(estudante, boletim):
    ...
    mediana1, mediana2, mediana3, mediana4, mediana5, mediana6 = (
        round(median(ind1), 1),
        round(median(ind2), 1),
        round(median(ind3), 1),
        round(median(ind4), 1),
...
class UpdateView(LoginRequiredMixin, LogMixin, generic.UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["goal_file"] = str(meta_geral.slug)
        context["mimeTypes"] = valid_formats
        context["resource"] = get_object_or_404(
            Bulletin, slug=self.kwargs.get("slug", "")
        )
...
class StatisticsView(LoginRequiredMixin, LogMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        vis_ou = Log.objects.filter(
            context__contains={"bulletin_id": bulletin.id},
            resource="bulletin",
            action="view",
            ...
            data_history.append(
                [
                    ...
                    ", ".join(
                        [
                            str(x)
                            for x in bulletin.topic.subject.group_subject.filter(
...
class SendMessage(LoginRequiredMixin, LogMixin, generic.edit.FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["bulletin"] = get_object_or_404(
            Bulletin, slug=self.kwargs.get("slug", "")
        )
        return context
