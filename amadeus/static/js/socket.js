...
function muralNotificationPost(content) {
  ...
  if ((render && page.indexOf(content.post_type) != -1) ||
      (render && content.post_type == 'general') || (render && is_resource)) {
    ...
    $('.mural-tabs').find('li').each(function() {
      ...
      if (identity == content.post_type) {
        var span = $(this).find('span'), actual = span.text();
...
function muralNotificationComment(content) {
  var page = window.location.pathname,
      render = (content.paths.indexOf(page) != -1),
      is_resource = (page.indexOf('resource') != -1), checker = 'general';
  ...
  if ((render && page.indexOf(checker) != -1) ||
      (render && content.post_type == 'generalpost' &&
       page.indexOf('categories') == -1 && page.indexOf('subjects') == -1) ||
      (render && is_resource)) {
    var section = $(content.container);
    ...
    $('.mural-tabs').find('li').each(function() {
      ...
      if ((identity == checker) ||
          (identity == 'general' && content.post_type == 'generalpost')) {
        var span = $(this).find('span'), actual = span.text();
    ...
    if (content.post_type == 'subjectpost') {
      var subject_mbadge =
              $('#subject_' + content.type_slug).find('.mural_notify'),
          actual = subject_mbadge.text();
...
function messageReceived(content) {
  ...
  if (talk_modal.is(':visible')) {
    ...
    $.ajax({type: 'GET', url: content.view_url});
  } else {
