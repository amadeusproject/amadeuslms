...
class CreateView(LoginRequiredMixin, LogMixin, generic.edit.CreateView):
    ...
    def get_success_url(self):
        messages.success(
            self.request,
            _(
                'The Youtube Video "%s" was added to the Topic "%s" of the virtual environment "%s" successfully!'
            )
            % (
                self.object.name,
        ...
        if self.object.show_window:
            ...
            self.request.session["resources"]["new_page_url"] = reverse(
                "youtube:window_view", kwargs={"slug": self.object.slug}
            )
...
class UpdateView(LoginRequiredMixin, LogMixin, generic.UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["resource"] = get_object_or_404(
            YTVideo, slug=self.kwargs.get("slug", "")
        )
    ...
    def get_success_url(self):
        messages.success(
            self.request,
            _('The YouTube Video "%s" was updated successfully!') % (self.object.name),
        )
...
class StatisticsView(LoginRequiredMixin, LogMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
            ...
            data_history.append(
                [
                    ...
                    ", ".join(
                        [
                            str(x)
                            for x in ytvideo.topic.subject.group_subject.filter(
        ...
        column_view, column_watch, column_finish = (
            str(_("View")),
            str(_("Watch")),
            str(_("Finish")),
        )
        ...
        not_watch = alunos.exclude(
            email__in=[
                log.user_email
                for log in vis_ou.filter(action="watch").distinct("user_email")
            ]
        )
...
class SendMessage(LoginRequiredMixin, LogMixin, generic.edit.FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["ytvideo"] = get_object_or_404(
            YTVideo, slug=self.kwargs.get("slug", "")
        )
        return context
