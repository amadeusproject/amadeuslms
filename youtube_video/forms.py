...
class YTVideoForm(forms.ModelForm):
    ...
    def clean_name(self):
        ...
        for topic in topics:
            ...
            if same_name > 0:
                self._errors["name"] = [
                    _("This subject already has a resource with this name")
                ]
    ...
    def clean_url(self):
        ...
        if url[0].lower() != "h":
            url = "https://" + url
        if (
            re.compile("https?://w{3}\.youtube\.com/").match(url) == None
            and re.compile("https?://youtu\.be/").match(url) == None
        ) or requests.get(url).status_code == 404:
