...
function cloud() {
  ...
  let width = dimensions.w -
      $('#cloudy').css('padding-left').match(/[0-9]+/)[0] -
      $('#cloudy').css('padding-right').match(/[0-9]+/)[0];

  width = width === 0 ? 775 : width;

  const height = (width * 1) / 2 > 360 ? 360 : width / 2 < 50 ? 50 : width / 2;

  $.get($('#cloudy').data('url'), jsonData => {
    let data = jsonData['cloud'];

    if (avatar !== null) {
      avatar.setCloudAudios(jsonData['info'], jsonData['tips']);
    }
    ...
    const dataconfig = {
      ...
      filltext: (a, transition) => {
        ...
        a.wordsv5.transition()
            ...
            .attr(
                'fill',
                (d, i) => a.fillpattern(a.scalePercent(d.myvalue, d.value)));
      },
    };
