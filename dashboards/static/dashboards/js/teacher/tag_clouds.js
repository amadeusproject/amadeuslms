...
function makeTableResources(data, nrows) {
  ...
  data.forEach((item, index) => {
    ...
    line = `${line}<td style="white-space: inherit"><a href='${
        item.access_url}'>${item.resource_name}</a></td>`;
  ...
  $('#resources_pag .previous').click(() => {
    ...
    if (page >= 10) {
      [...Array(pages_resources_table).keys()].forEach(i => {
        const page = `<li class='page-item page-item-number' data-page=${
            i + 1}>${i + 1}</li>`;
...
function makeTable(data, tableId, paginationId, nrows) {
  ...
  [...Array(pages_tags).keys()].forEach(i => {
    const page_tag = `<li class='page-item page-item-number' data-page=${
        i + 1}>${i + 1}</li>`;
...
function makeTagTable(data, nrows) {
  ...
  data.forEach((item, index) => {
    ...
    line = `${line}<td><a class='tag_table_resources' data-link='${
        item.link}'>${item.key}</a></td>`;
  ...
  $('#tag-pagination .previous').click(() => {
    ...
    $('#tag-pagination .page-item-number').removeClass('active');
    $(`#tag-pagination .page-item-number[data-page=${page}]`)
        .addClass('active');
  ...
  $('#tag-access .tag_table_resources').on('click', e => {
    ...
    $.get(link, dataset => {
      dataset = dataset.sort((d1, d2) => {
        ...
        return d1.qtd_access < d2.qtd_access ?
            1 :
...
function cloud(url, dataIn, dataEnd) {
  ...
  const height =
      (width * 1.1) / 2 > 360 ? 360 : width / 2 < 50 ? 50 : width / 2;
  ...
  $.get(url, {data_ini: dataIn, data_end: dataEnd}, data => {
    ...
    data.sort(
        (d1, d2) => (d1.value > d2.value ? -1 : d1.value < d2.value ? 1 : 0));
    ...
    const dataconfig = {
      ...
      filltext: (a, transition) => {
        ...
        a.wordsv5.transition()
            .duration(transition)
            .attr(
                'fill',
                (d, i) => a.fillpattern(a.scalePercent(d.value, d.value)));
      },
    };
