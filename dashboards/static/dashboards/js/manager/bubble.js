...
class BubbleChart {
  ...
  create(chartConfig) {
    ...
    for (let i = 0; i < a.chartConfig.data.length; i++) {
      ...
      if (a.data[index].link_profile === undefined) a.data[index].link_profile = a.chartConfig.data[i].link_profile;
      if (a.data[index].link_chat === undefined) a.data[index].link_chat = a.chartConfig.data[i].link_chat;
      ...
      if (a.data[index].status === undefined) a.data[index].status = a.chartConfig.data[i].status;
      if (a.data[index].status_text === undefined) a.data[index].status_text = a.chartConfig.data[i].status_text;
    ...
    this.bubbles = this.g.selectAll(".user-dot").data(a.nodes).enter().append("g").attr("class", "user-dot");
...
function makeTable(dataset, nrows) {
  ...
  dataset.forEach((item, index) => {
    ...
    line = `${line}<td><a class='userPopover' data-user='${JSON.stringify(item)}' style='cursor: pointer'>${
      item.user
    }</a></td>`;
  ...
  $("#students_pag .previous").click(() => {
    ...
    if (page > 11) {
      [...Array(pages).keys()].forEach(i => {
        if (i < page + 5 && i > page - 5) $(`#students_pag .page-item-number[data-page=${i + 1}]`).show();
      });
    } else {
      [...Array(pages).keys()].forEach(i => {
        if (i > 10) $(`#students_pag .page-item-number[data-page=${i + 1}]`).hide();
        else $(`#students_pag .page-item-number[data-page=${i + 1}]`).show();
      });
    }
    const init = page * nrows - nrows;
    const end = page * nrows;
  ...
  $("#students_pag .next").click(() => {
    ...
    if (page > 11) {
      [...Array(pages).keys()].forEach(i => {
        if (i < page + 5 && i > page - 5) $(`#students_pag .page-item-number[data-page=${i + 1}]`).show();
        else if (i < pages - 10) $(`#students_pag .page-item-number[data-page=${i + 1}]`).hide();
      });
    }
