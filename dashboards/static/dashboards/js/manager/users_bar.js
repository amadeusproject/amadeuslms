...
function drawBarChart(data){
    ...
    $("#number-students-active").html(
        `<p data-toggle="tooltip" data-placement="bottom" title="${data["active_students"]} dos ${data["total_students"]} estudantes estão ativos no período selecionado">${data["active_students"]}</p>`
    );
    $("#number-teacher-active").html(
        `<p data-toggle="tooltip" data-placement="bottom" title="${data["active_teachers"]} dos ${data["total_teachers"]} professores estão ativos no período selecionado">${data["active_teachers"]}</p>`
    );
    ...
    $("#number-students-inactive").html(
        `<p data-toggle="tooltip" data-placement="bottom" title="${inactive_students} dos ${data["total_students"]} estudantes estão inativos no período selecionado">${inactive_students}</p>`
    );
    $("#number-teacher-inactive").html(
        `<p data-toggle="tooltip" data-placement="bottom" title="${inactive_teachers} dos ${data["total_teachers"]} professores estão inativos no período selecionado">${inactive_teachers}</p>`
    );
