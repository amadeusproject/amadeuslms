...
class HeatMap {
  ...
  validData(chartConfig) {
    ...
    if (a.chartConfig.name === undefined ||
    ...
    if (!a.chartConfig.chart.calendar) {
      ...
      if (a.chartConfig.calendar.parent === undefined &&
          a.chartConfig.calendar.svg != true) {
    ...
    if (a.chartConfig.cornerLabel !== undefined) {
      if (a.chartConfig.hour && a.chartConfig.hour.texts.corner === undefined &&
  ...
  create() {
    ...
    if (!a.chartConfig.chart.calendar) {
      this.calendar.svg = a.chartConfig.calendar.svg &&
    ...
    if (!a.chartConfig.chart.hour) {
      ...
      this.hour.collapse =
          this.hour.rectsContent.append('g').attr('id', 'collapse');
      this.hour.collapse.append('path');
      this.hour.collapse.append('rect').attr('opacity', 0);
      this.hour.totalRects = this.hour.rectsContent.selectAll('.week')
      ...
      if (a.chartConfig.centerLabel !== undefined) {
        this.hour.totalRects.append('text')
            .attr('class', 'center')
  ...
  createChart(chart, chartConfig) {
    ...
    chart.rects = chart.rectsContent.selectAll('.month')
                      .data(chart.data)
                      .enter()
                      .append('g')
                      .attr('class', 'month');
  ...
  draw() {
    ...
    if (!a.chartConfig.chart.calendar) {
      ...
      const temp = String.adjustWidth(
          '00/XXX', a.chartConfig.layout.font_size, a.width * 0.1);
    ...
    if (!a.chartConfig.chart.hour) {
      ...
      this.hour.margin.top = Math.max(
          a.hour.margin.top,
          a.chartConfig.hour.axis.day.all ? 20 :
                                            11 + 1.05 * a.font_size +
    ...
    this.nVerticalRects = a.calendar.height +
        (a.calendar.height === 0 ? a.hour.height : a.hour.height / 1.5);

    this.size = a.chartConfig.dimensions.height - a.calendar.margin.top -
        a.calendar.margin.bottom - a.hour.margin.top - a.hour.margin.bottom;

    this.size = a.size / a.nVerticalRects;

    const tempSize = (a.chartConfig.dimensions.width -
    ...
    a.height = a.size * this.nVerticalRects + a.calendar.margin.top +
        a.calendar.margin.bottom + a.hour.margin.top + a.hour.margin.bottom;
    ...
    if (!a.chartConfig.chart.calendar) {
      const temp = String.adjustWidth(
          '00/XXX', a.chartConfig.layout.font_size,
    ...
    if (!a.chartConfig.chart.hour) {
      ...
      if (!a.chartConfig.chart.calendar) {
        this.hour.size /= 1.5;
        temp += this.calendar.margin.top + this.calendar.margin.bottom +
            this.size * a.calendar.height;
      }

      this.hour.margin.top += -(a.chartConfig.chart.calendar ? desloc -
                                        (a.chartConfig.title === undefined ?
                                             0 :
                                             a.chartConfig.title.desloc) :
                                                               0) +
          temp;
      this.hour.verticalFunction = MyDate.dayVal;
    }

    this.day = d3.scaleBand()
    ...
    if (!a.chartConfig.chart.hour) {
      ...
      const translatebefore = a.hour.g.attr('transform')
                                  .replace('translate(', '')
                                  .replace(')', '')
                                  .split(',');
      a.hour.g.attr(
      ...
      this.hour.totalRects
          ...
          .attr(
              'transform',
              (d, i) => `translate(${a.day(MyDate.weekName()[i])}, ${
      ...
      d3.select('body').on('keydown', event => {
        if (a.calendar.on) {
          if (d3.event.keyCode == 38) {
            if (a.scrolMove(
                    a.calendar.scrolposition - 1, a.calendar,
                    a.chartConfig.calendar))
            ...
            if (a.scrolMove(
                    a.calendar.scrolMax, a.calendar, a.chartConfig.calendar))
              d3.event.preventDefault();
        ...
        if (a.hour.on) {
          if (d3.event.keyCode == 38) {
            if (a.scrolMove(
                    a.hour.scrolposition - 1, a.hour, a.chartConfig.hour))
              d3.event.preventDefault();
          } else if (d3.event.keyCode == 40) {
            if (a.scrolMove(
                    a.hour.scrolposition + 1, a.hour, a.chartConfig.hour))
  ...
  drawChart(chart, chartConfig) {
    ...
    if (!chartConfig.axis.day.all) {
      chart.dayAxis.transition()
          .transition(500)
          .call(d3.axisTop(a.day))
          .attr('font-size', a.chartConfig.layout.font_size2);
      chart.dayAxis.selectAll('text').attr('y', 0).transition().text(
          (d, i) => String.adjustLength(
              MyDate.weekName()[i], a.chartConfig.layout.font_size2,
    ...
    chart.rects.attr(
        'transform',
        (d, i) => `translate(${a.day(MyDate.weekName()[d.dayOfWeek])}, ${
            chart.vertical(
    ...
    if (chart.domain.length > chartConfig.extrapolation) {
      ...
      chart.rects.transition()
          ...
          .attr(
              'opacity',
              d => {
                if (chart.vertical(chart.verticalFunction(chart.domain, d)) >
                    location) {
                  return 0;
                }
          ...
          .attr('transform', d => {
            ...
            if (temp > location2 || temp < -2 * chart.vertical.bandwidth()) {
              return `translate(${- 3 * a.day.bandwidth()}, ${
                  chart.vertical(chart.verticalFunction(chart.domain, d))})`;
            }

            return `translate(${a.day(MyDate.weekName()[d.dayOfWeek])}, ${
                chart.vertical(
                    chart.verticalFunction(chart.domain, d),
                    )})`;
          });
  ...
  scrolEvents(chart, chartConfig) {
    ...
    chart.extTriangles.attr(
        'transform',
        `translate(${- chart.margin.left / 2}, ${
            chart.vertical(chart.vertical.domain()[0])})`,
    );

    const y = chart.vertical(chart.domain[0]) +
        chartConfig.extrapolation * chart.size2 - 0.25 * chart.size2;
    ...
    const lineFunction =
        d3.line().x(d => d.x).y(d => d.y).curve(d3.curveLinearClosed);

    chart.extTriangles.select('.up')
        .attr('opacity', 0)
    ...
    chart.extTriangles.select('.down')
        .attr('opacity', 0)
    ...
    chart.extTriangles.select('.down').transition().duration(500).attr(
        'opacity', 1);
  ...
  scrolMove(position, chart, chartConfig) {
    ...
    if (chart.extrapolation) {
      ...
      chart.scrolposition = chart.scrolposition > chart.scrolMax ?
          chart.scrolMax :
          chart.scrolposition;
      ...
      chart.extTriangles.select('.up').attr(
          'opacity', chart.scrolposition === 0 ? 0 : 1);
      chart.extTriangles.select('.down').attr(
          'opacity', chart.scrolposition === chart.scrolMax ? 0 : 1);
  ...
  hourView() {
    ...
    if (!a.chartConfig.chart.hour) {
      ...
      if (a.hour.hourShow) {
        a.hour.rects.transition()
            .delay(0)
            .duration(500)
            .attr(
                'transform',
                d => `translate(${a.day(MyDate.weekName()[d.dayOfWeek])}, ${
                    a.hour.vertical(
        ...
        if (!a.chartConfig.hour.axis.vertical.all) {
          a.hour.verticalAxis.selectAll('.tick')
              .transition()
              .delay(0)
              .duration(500)
              .attr(
                  ...
                  d => `translate(0, ${
                      a.hour.vertical(
                          a.hour.vertical.domain()
                              [a.hour.scrolposition ? a.hour.scrolposition :
                                                      0]) -
                      a.hour.size2 / 2})`,
                  )
              .attr('opacity', 0);
        ...
        if (a.hour.extrapolation) {
          a.hour.extTriangles.selectAll('path')
              .transition()
              .delay(0)
              .duration(500)
              .attr('opacity', 0);
        ...
        if (!a.chartConfig.hour.axis.vertical.all) {
          a.hour.verticalAxis.selectAll('.tick')
              ...
              .attr('transform', d => {
                ...
                if (temp > location2 ||
                    temp < -2 * a.hour.vertical.bandwidth()) {
                  return `translate(${- 2 * a.day.bandwidth()}, ${
                      a.hour.vertical(
                          a.hour.verticalFunction(a.hour.domain, d)) +
        ...
        if (a.hour.extrapolation) {
          a.hour.extTriangles.select('.down').attr(
              'opacity', a.hour.scrolposition === a.hour.scrolMax ? 0 : 1);
        }
      }

      a.hour.hourShow = !a.hour.hourShow;
      a.hour.collapse.select('path')
          .transition()
          .duration(500)
          ...
          .attr(
              'transform',
              `translate(${
                  a.day(a.day.domain()[0]) +
                  a.day.bandwidth() * 0.5}, ${a.size * 0.3})`);
    }
  }
  ...
  dataSetter() {
    ...
    if (!a.chartConfig.chart.hour) {
      ...
      for (let i = 0; i < 7 * model; i++) {
        ...
        if (model === 4) {
          ...
          this.hour.data[i].hour = `${this.hour.data[i].hour < 10 ? '0' : ''}${
              this.hour.data[i].hour}h-${
              this.hour.data[i].hour + 6 < 10 ?
                  '0' :
        ...
        this.hour.data[i].toString = function() {
          return `${MyDate.weekName(this.dayOfWeek)},${
              MyDate.dayVal(MyDate.hourNames(undefined, model), this)}`;
        };
      }
      ...
      for (let i = 0; i < a.data.length; i++) {
        const obj =
            `${a.data[i].dayOfWeek.toString()}-${a.data[i].user_id.toString()}`;
        ...
        if (!users_set.has(obj)) {
          ...
          if (a.chartConfig.hour.model === 4) {
            this.hour
                .data[a.data[i].dayOfWeek * 4 + parseInt(a.data[i].hour / 6)]
    ...
    if (!a.chartConfig.chart.calendar) {
      ...
      for (i, j = MyDate.sum(j, start); i < months;
        ...
        for (j.value = 0; k < days[i]; k++) {
          ...
          while (a.data[indexData] &&
                 MyDate.greatThan(a.data[indexData], j, true) === 0) {
            j.value += a.data[indexData].value;
            indexData++;
          }

          this.calendar.data.push(new MyDate(
              j.year, j.month, j.day, undefined, j.value, j.dayOfWeek));
  ...
  titleConstruct() {
    ...
    if (a.chartConfig.title !== undefined) {
      if (this.titleElement === undefined) {
        this.titleElement =
            this.svg.append('text')
                ...
                .text(
                    () => d3.textData(
                        a.chartConfig.dataConfig, a.chartConfig.title.text));
      }

      this.titleElement.style(
          'font-size',
          String.adjustWidth(
              this.titleElement.text(), a.chartConfig.title.font.size, a.width),
      );
      ...
      const labelDim =
          document.getDimensions(`#titleElement_${this.chartConfig.name}`);
