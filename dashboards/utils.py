...
def done_percent(pendency):
    ...
    notified = Notification.objects.filter(user__in=users.values_list('id', flat=True),
                                           creation_date=datetime.now(), task=pendency).count()
...
def get_pend_graph(user, subject):
    ...
    for pendency in pendencies:
        ...
        item["date"]["delay"] = formats.date_format(pendency.limit_date,
                                                    "m/d/Y H:i") if pendency.limit_date else "infinity"
        ...
        if user in users:
            has_action = Log.objects.filter(user_id=user.id, action=pend_action,
                                            resource=resource_type,
                                            context__contains={resource_key: resource_id},
                                            datetime__date__gte=subject_begin_date).exists()
...
def getAccessedTags(subject, user):
    ...
    logs = Log.objects.filter(datetime__date__gte=timezone.now() - timedelta(days=7),
                              datetime__date__lt=timezone.now())
    ...
    for tag in tags:
        if not tag.name == '':
            ...
            item = {"tag_name": tag.name, "details_url": reverse('dashboards:tag_accessess', args=(
                tag.id, subject.slug, user.email,), kwargs={})}
...
def getTagAccessess(subject, tag, user):
    ...
    for resource in resources:
        ...
        history = logs.filter(component='resources',
                              context__contains={resource._my_subclass + '_id': resource.id})
...
def getOtherIndicators(subject, user):
    ...
    sub_access = logs.filter(Q(component='subject') & Q(resource='subject') & Q(
        context__contains={'subject_id': subject.id}) & (Q(action='access') | Q(action='view')))

    if sub_access:
        # Subject access
        item = {"max_access":
                    sub_access.values('user_id').annotate(c_user=Count('user_id')).aggregate(
                        Max('c_user'))['c_user__max'],
                "my_access": sub_access.filter(user_id=user.id).count()}
        ...
        item = {'max_access': max([len(x[1]) for x in result.items()]) if result else 0,
                'my_access': len(result[user.id]) if user.id in result else 0}

        data.append(item)
    else:
        item = {'max_access': 0, 'my_access': 0}

        data.append(item)
        data.append(item)

    # Resources access
    resources_access = logs.filter(component='resources',
                                   context__contains={'subject_id': subject.id})

    if resources_access:
        item = {"max_access":
                    resources_access.values('user_id').annotate(c_user=Count('user_id')).aggregate(
                        Max('c_user'))['c_user__max'],
                "my_access": resources_access.filter(user_id=user.id).count()}
    else:
        item = {'max_access': 0, 'my_access': 0}
    ...
    query = resources_access.extra({ \
        ...
        }).values('bulletins', 'filelinks', 'goals', 'links', 'pdffiles', 'questionarys',
                  'ytvideos', 'webconferences', 'webpages', 'user_id') \
    ...
    item["my_access"] = resources_access.filter(user_id=user.id).extra({ \
        ...
        'webpages': "context->>'webpage_id'", \
                  ...
                  'ytvideos', 'webconferences', 'webpages') \
        .annotate(id_count=Count('id', distinct=True)).order_by().count()

    data.append(item)

    # Resources in time
    pend = Pendencies.objects.filter(resource__topic__subject=subject.id, resource__visible=True,
                                     end_date__date__lt=timezone.now(),
                                     end_date__date__gte=timezone.now() - timedelta(days=7))
    ...
    if pend.count() > 0:
        ...
        for p in pend:
            conds.add((Q(context__contains={p.resource._my_subclass + '_id': p.resource.id}) & Q(
                action=p.action)), Q.OR)

        res_access = logs.filter(conds)

        item["max_access"] = \
        res_access.values('user_id').annotate(c_user=Count('user_id')).aggregate(Max('c_user'))[
            'c_user__max'] if res_access else 0
