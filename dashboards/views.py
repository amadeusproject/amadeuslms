...
class GeneralView(generic.TemplateView):
    ...
    def get_last_twelve_months(self):
        today = date.today()
        months = []
        month_mappings = {1: _('January'), 2: _('February'), 3: _('March'), 4: _('April'),
                          5: _('May'), 6: _('June'), 7: _('July')
            , 8: _('August'), 9: _('September'), 10: _('October'), 11: _('November'),
                          12: _('December')}
...
class CategoryView(generic.TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        if Category.objects.filter(
                coordinators__id=self.request.user.id).exists() or self.request.user.is_staff:
...
class LogView(generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['style_files'] = ['dashboards/css/jquery.dataTables.min.css',
...
def parse_log_queryset_to_JSON(logs):
    data = []
    for log in logs:
        datum = {'user': log.user, 'resource': log.resource,
                 'datetime': log.datetime.strftime("%d/%m/%Y %H:%M"), 'action': log.action,
                 'context': log.context, 'component': log.component}
        data.append(datum)
    return data
    ...
    def get_context_data(self, **kwargs):
        ...
        if has_subject_permissions(self.request.user, subject):
            ...
            if student is not None:
                ...
                context["metrics_url"] = reverse('dashboards:other_metrics',
                                                 args=(subject.slug, student.email,), kwargs={})
            else:
