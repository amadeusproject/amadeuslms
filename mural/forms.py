...
class Validation(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        CHOICES = (("comment", pgettext_lazy("form", "Comment")),
                   ("help", pgettext_lazy("form", "Ask for Help")))
...
class SubjectPostForm(Validation):
    def __init__(self, *args, **kwargs):
        ...
        if user.is_staff:
            self.fields['resource'].choices = [(r.id, str(r)) for r in
                                               Resource.objects.filter(Q(topic__subject=subject))]
        else:
            self.fields['resource'].choices = [(r.id, str(r)) for r in Resource.objects.filter(
                Q(topic__subject=subject) & (Q(all_students=True) | Q(students=user) | Q(
                    groups__participants=user)))]
