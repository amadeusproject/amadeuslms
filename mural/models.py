...
class Mural(KnowsChild):
    action = models.CharField(
        ...
        default="comment",
        choices=(
            ("comment", pgettext_lazy("modal", "Comment")),
            ("help", pgettext_lazy("modal", "Ask for Help")),
        ),
    )
    ...
    user = models.ForeignKey(
        User, verbose_name=_("User"), related_name="post_user", null=True
    )
...
class SubjectPost(Mural):
    space = models.ForeignKey(
        Subject, verbose_name=_("Subject"), related_name="post_subject"
    )
    resource = models.ForeignKey(
        Resource,
        verbose_name=_("Resource"),
        related_name="post_resource",
        null=True,
        blank=True,
    )
...
class Comment(models.Model):
    ...
    image = models.ImageField(
        ...
        null=True,
        blank=True,
        upload_to="posts/comments/",
        validators=[validate_img_extension],
    )
    post = models.ForeignKey(
        Mural, verbose_name=_("Post"), related_name="comment_post", null=True
    )
    user = models.ForeignKey(
        User, verbose_name=_("User"), related_name="comment_user", null=True
    )
    create_date = models.DateTimeField(_("Create Date"), auto_now_add=True)
    last_update = models.DateTimeField(_("Last Update"), auto_now=True)
    edited = models.BooleanField(_("Edited"), default=False)
...
class MuralVisualizations(models.Model):
    ...
    post = models.ForeignKey(
        Mural, verbose_name=_("Post"), related_name="visualization_post", null=True
    )
    comment = models.ForeignKey(
        Comment,
        verbose_name=_("Comment"),
        related_name="visualization_comment",
        null=True,
    )
    user = models.ForeignKey(
        User, verbose_name=_("User"), related_name="visualization_user", null=True
    )
...
class MuralFavorites(models.Model):
    post = models.ForeignKey(
        Mural, verbose_name=_("Post"), related_name="favorites_post", null=True
    )
    user = models.ForeignKey(
        User, verbose_name=_("User"), related_name="favorites_user", null=True
    )
