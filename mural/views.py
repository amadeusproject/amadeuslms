...
class GeneralIndex(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        ...
        if not favorites:
            if mines:
                general = GeneralPost.objects.extra(select={
                    "most_recent": "greatest(last_update, (select max(mural_comment.last_update) from mural_comment where mural_comment.post_id = mural_generalpost.mural_ptr_id))"}).filter(
                    mural_ptr__user=user)
            else:
                general = GeneralPost.objects.extra(select={
                    "most_recent": "greatest(last_update, (select max(mural_comment.last_update) from mural_comment where mural_comment.post_id = mural_generalpost.mural_ptr_id))"})
        else:
            if mines:
                general = GeneralPost.objects.extra(select={
                    "most_recent": "greatest(last_update, (select max(mural_comment.last_update) from mural_comment where mural_comment.post_id = mural_generalpost.mural_ptr_id))"}).filter(
                    favorites_post__isnull=False, favorites_post__user=user, mural_ptr__user=user)
            else:
                general = GeneralPost.objects.extra(select={
                    "most_recent": "greatest(last_update, (select max(mural_comment.last_update) from mural_comment where mural_comment.post_id = mural_generalpost.mural_ptr_id))"}).filter(
                    favorites_post__isnull=False, favorites_post__user=user)
        ...
        if not page:  # Don't need this if is pagination
            general_visualizations = MuralVisualizations.objects.filter(
                Q(user=user) & Q(viewed=False) & (Q(post__generalpost__isnull=False) | Q(
                    comment__post__generalpost__isnull=False))).distinct()

            self.totals['general'] = general_visualizations.count()
            self.totals['category'] = MuralVisualizations.objects.filter(
                Q(user=user) & Q(viewed=False) & ((Q(user__is_staff=True) & (
                        Q(post__categorypost__isnull=False) | Q(
                    comment__post__categorypost__isnull=False))) | Q(
                    post__categorypost__space__coordinators=user) | Q(
                    comment__post__categorypost__space__coordinators=user) | Q(
                    post__categorypost__space__subject_category__students=user) | Q(
                    comment__post__categorypost__space__subject_category__students=user) | Q(
                    post__categorypost__space__subject_category__professor=user) | Q(
                    comment__post__categorypost__space__subject_category__professor=user))).distinct().count()
            self.totals['subject'] = MuralVisualizations.objects.filter(
                Q(user=user) & Q(viewed=False) & ((Q(user__is_staff=True) & (
                        Q(post__subjectpost__isnull=False) | Q(
                    comment__post__subjectpost__isnull=False))) | Q(
                    post__subjectpost__space__professor=user) | Q(
                    comment__post__subjectpost__space__professor=user) | Q(
                    post__subjectpost__space__students=user) | Q(
                    comment__post__subjectpost__space__students=user))).distinct().count()

            general_visualizations.update(viewed=True, date_viewed=datetime.now())

            MuralVisualizations.objects.filter(user=user, viewed=False,
                                               comment__post__generalpost__isnull=False).update(
                viewed=True, date_viewed=datetime.now())
...
class GeneralCreate(LoginRequiredMixin, generic.edit.CreateView):
    ...
    def get_context_data(self, *args, **kwargs):
        context = super(GeneralCreate, self).get_context_data(**kwargs)
...
class GeneralUpdate(LoginRequiredMixin, generic.UpdateView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['form_url'] = reverse_lazy("mural:update_general", args=(),
...
class GeneralDelete(LoginRequiredMixin, generic.DeleteView):
    ...
    def get_context_data(self, *args, **kwargs):
        context = super(GeneralDelete, self).get_context_data(**kwargs)

        context['form_url'] = reverse_lazy("mural:delete_general", args=(),
                                           kwargs={'pk': self.object.id})
...
def load_category_posts(request, category):
    ...
    if not favorites:
        if mines:
            posts = CategoryPost.objects.extra(select={
                "most_recent": "greatest(last_update, (select max(mural_comment.last_update) from mural_comment where mural_comment.post_id = mural_categorypost.mural_ptr_id))"}).filter(
                space__id=category, mural_ptr__user=user)
        else:
            posts = CategoryPost.objects.extra(select={
                "most_recent": "greatest(last_update, (select max(mural_comment.last_update) from mural_comment where mural_comment.post_id = mural_categorypost.mural_ptr_id))"}).filter(
                space__id=category)
        ...
        if mines:
            posts = CategoryPost.objects.extra(select={
                "most_recent": "greatest(last_update, (select max(mural_comment.last_update) from mural_comment where mural_comment.post_id = mural_categorypost.mural_ptr_id))"}).filter(
                space__id=category, favorites_post__isnull=False, favorites_post__user=user,
                mural_ptr__user=user)
        else:
            posts = CategoryPost.objects.extra(select={
                "most_recent": "greatest(last_update, (select max(mural_comment.last_update) from mural_comment where mural_comment.post_id = mural_categorypost.mural_ptr_id))"}).filter(
    ...
    if has_page is None:
        views = MuralVisualizations.objects.filter(Q(user=user) & Q(viewed=False) & (
                Q(comment__post__categorypost__space__id=category) | Q(
            post__categorypost__space__id=category)))
        n_views = views.count()
    ...
    return JsonResponse({"posts": response, "unviewed": n_views, "count": posts.count(),
                         "num_pages": paginator.num_pages, "num_page": page_obj.number})
    ...
    def get_queryset(self):
        ...
        if user.is_staff:
            categories = Category.objects.extra(select={
                "has_message": "select 1 FROM mural_muralvisualizations AS visul LEFT JOIN mural_categorypost AS post ON visul.post_id = post.mural_ptr_id LEFT JOIN mural_comment AS com ON visul.comment_id = com.id LEFT JOIN mural_categorypost AS post2 ON com.post_id = post2.mural_ptr_id  WHERE visul.viewed = FALSE AND visul.user_id = %s AND (post.space_id = categories_category.id OR post2.space_id = categories_category.id) LIMIT 1"},
                select_params=(user.id,)).order_by('has_message',
                                                   'name')
        else:
            categories = Category.objects.extra(select={
                "has_message": "select 1 FROM mural_muralvisualizations AS visul LEFT JOIN mural_categorypost AS post ON visul.post_id = post.mural_ptr_id LEFT JOIN mural_comment AS com ON visul.comment_id = com.id LEFT JOIN mural_categorypost AS post2 ON com.post_id = post2.mural_ptr_id  WHERE visul.viewed = FALSE AND visul.user_id = %s AND (post.space_id = categories_category.id OR post2.space_id = categories_category.id) LIMIT 1"},
                select_params=(user.id,)).filter(
                Q(coordinators__pk=user.pk) | Q(subject_category__professor__pk=user.pk) | Q(
                    subject_category__students__pk=user.pk, visible=True)).distinct().order_by(
                'has_message', 'name')

        self.totals['general'] = MuralVisualizations.objects.filter(
            Q(user=user) & Q(viewed=False) & (Q(post__generalpost__isnull=False) | Q(
                comment__post__generalpost__isnull=False))).distinct().count()
        self.totals['category'] = MuralVisualizations.objects.filter(
            Q(user=user) & Q(viewed=False) & ((Q(user__is_staff=True) & (
                    Q(post__categorypost__isnull=False) | Q(
                comment__post__categorypost__isnull=False))) | Q(
...
class CategoryCreate(LoginRequiredMixin, generic.edit.CreateView):
    ...
    def form_valid(self, form):
        ...
        simple_notify = _("%s has made a post in %s") % (
            str(self.object.user), str(self.object.space))
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['form_url'] = reverse_lazy("mural:create_category", args=(),
...
class CategoryUpdate(LoginRequiredMixin, generic.UpdateView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['form_url'] = reverse_lazy("mural:update_category", args=(),
                                           kwargs={'pk': self.object.id})
...
class CategoryDelete(LoginRequiredMixin, generic.DeleteView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['form_url'] = reverse_lazy("mural:delete_category", args=(),
                                           kwargs={'pk': self.object.id})
        context['message'] = _('Are you sure you want to delete this post?')
...
def load_subject_posts(request, subject):
    ...
    if has_page is None:
        views = MuralVisualizations.objects.filter(Q(user=user) & Q(viewed=False) & (
                Q(comment__post__subjectpost__space__id=subject) | Q(
            post__subjectpost__space__id=subject)))
        n_views = views.count()
        views.update(viewed=True, date_viewed=datetime.now())
...
class SubjectIndex(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        ...
        if user.is_staff:
            subjects = Subject.objects.extra(select={
                "has_message": "select 1 FROM mural_muralvisualizations AS visul LEFT JOIN mural_subjectpost AS post ON visul.post_id = post.mural_ptr_id LEFT JOIN mural_comment AS com ON visul.comment_id = com.id LEFT JOIN mural_subjectpost AS post2 ON com.post_id = post2.mural_ptr_id  WHERE visul.viewed = FALSE AND visul.user_id = %s AND (post.space_id = subjects_subject.id OR post2.space_id = subjects_subject.id) LIMIT 1"},
                select_params=(user.id,)).order_by('has_message',
                                                   'name')
        else:
            subjects = Subject.objects.extra(select={
                "has_message": "select 1 FROM mural_muralvisualizations AS visul LEFT JOIN mural_subjectpost AS post ON visul.post_id = post.mural_ptr_id LEFT JOIN mural_comment AS com ON visul.comment_id = com.id LEFT JOIN mural_subjectpost AS post2 ON com.post_id = post2.mural_ptr_id  WHERE visul.viewed = FALSE AND visul.user_id = %s AND (post.space_id = subjects_subject.id OR post2.space_id = subjects_subject.id) LIMIT 1"},
                select_params=(user.id,)).filter(
                Q(category__coordinators__pk=user.pk) | Q(professor__pk=user.pk) | Q(
                    students__pk=user.pk, visible=True)).distinct().order_by('has_message', 'name')
...
class SubjectCreate(LoginRequiredMixin, generic.edit.CreateView):
    ...
    def form_valid(self, form):
        ...
        if self.object.resource:
            paths.append(
                reverse("mural:resource_view", args=(), kwargs={'slug': self.object.resource.slug}))
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['form_url'] = reverse_lazy("mural:create_subject", args=(),
                                           kwargs={'slug': self.kwargs.get('slug', None)})
...
class SubjectUpdate(LoginRequiredMixin, generic.UpdateView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['form_url'] = reverse_lazy("mural:update_subject", args=(),
                                           kwargs={'pk': self.object.id})
...
class SubjectDelete(LoginRequiredMixin, generic.DeleteView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['form_url'] = reverse_lazy("mural:delete_subject", args=(),
                                           kwargs={'pk': self.object.id})
        context['message'] = _('Are you sure you want to delete this post?')
...
class SubjectView(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        ...
        if not page:  # Don't need this if is pagination
            MuralVisualizations.objects.filter(Q(user=user) & Q(viewed=False) & (
                    Q(post__subjectpost__space=subject) | Q(
                comment__post__subjectpost__space=subject))).update(viewed=True,
    ...
    def get_queryset(self):
        ...
        if not favorites:
            if mines:
                if not user.is_staff:
                    posts = SubjectPost.objects.extra(select={
                        "most_recent": "greatest(mural_mural.last_update, (select max(mural_comment.last_update) from mural_comment where mural_comment.post_id = mural_subjectpost.mural_ptr_id))"}).filter(
                        Q(resource=resource) & Q(mural_ptr__user=user) & (
                                ...
                                (Q(resource__isnull=False) & (Q(resource__all_students=True) | Q(
                                    resource__students=user) | Q(
                                    resource__groups__participants=user))))).distinct()
                else:
                    posts = SubjectPost.objects.extra(select={
                        "most_recent": "greatest(mural_mural.last_update, (select max(mural_comment.last_update) from mural_comment where mural_comment.post_id = mural_subjectpost.mural_ptr_id))"}).filter(
                        resource=resource, mural_ptr__user=user)
            else:
                if not user.is_staff:
                    posts = SubjectPost.objects.extra(select={
                        "most_recent": "greatest(mural_mural.last_update, (select max(mural_comment.last_update) from mural_comment where mural_comment.post_id = mural_subjectpost.mural_ptr_id))"}).filter(
            ...
            if mines:
                if not user.is_staff:
                    posts = SubjectPost.objects.extra(select={
                        "most_recent": "greatest(mural_mural.last_update, (select max(mural_comment.last_update) from mural_comment where mural_comment.post_id = mural_subjectpost.mural_ptr_id))"}).filter(
                        Q(resource=resource) & Q(favorites_post__isnull=False) & Q(
                            favorites_post__user=user) & Q(mural_ptr__user=user) & (
                                Q(space__category__coordinators=user) |
                                Q(space__professor=user) |
                                Q(resource__isnull=True) |
                                (Q(resource__isnull=False) & (Q(resource__all_students=True) | Q(
                    ...
                    posts = SubjectPost.objects.extra(select={
                        "most_recent": "greatest(mural_mural.last_update, (select max(mural_comment.last_update) from mural_comment where mural_comment.post_id = mural_subjectpost.mural_ptr_id))"}).filter(
                        resource=resource, favorites_post__isnull=False, favorites_post__user=user,
                        mural_ptr__user=user)
            else:
                if not user.is_staff:
                    posts = SubjectPost.objects.extra(select={
        ...
        if not page:  # Don't need this if is pagination
            MuralVisualizations.objects.filter(Q(user=user) & Q(viewed=False) & (
                ...
                comment__post__subjectpost__resource=resource))).update(viewed=True,
                                                                        date_viewed=datetime.now())
...
class ResourceCreate(LoginRequiredMixin, generic.edit.CreateView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['form_url'] = reverse_lazy("mural:create_resource", args=(),
                                           kwargs={'slug': self.kwargs.get('slug', None),
                                                   'rslug': self.kwargs.get('rslug', None)})
...
class CommentCreate(LoginRequiredMixin, generic.edit.CreateView):
    ...
    def form_valid(self, form):
        ...
        if post._my_subclass == "subjectpost":
            paths.append(
                reverse("mural:subject_view", args=(), kwargs={'slug': post.get_space_slug()}))

            if post.subjectpost.resource:
                paths.append(reverse("mural:resource_view", args=(),
                                     kwargs={'slug': post.subjectpost.resource.slug}))
        ...
        notification = {
            ...
            "user_icon": self.object.user.image_url,
            "simple_notify": simple_notify,
            "complete": render_to_string("mural/_view_comment.html", {"comment": self.object},
                                         self.request),
...
class CommentUpdate(LoginRequiredMixin, generic.UpdateView):
    ...
    def form_valid(self, form):
        ...
        if self.object.post._my_subclass == "subjectpost":
            paths.append(reverse("mural:subject_view", args=(),
                                 kwargs={'slug': self.object.post.get_space_slug()}))

            if self.object.post.subjectpost.resource:
                paths.append(reverse("mural:resource_view", args=(),
                                     kwargs={'slug': self.object.post.subjectpost.resource.slug}))
...
def get_context_data(self, *args, **kwargs):
    ...
    context['form_url'] = reverse_lazy("mural:update_comment", args=(),
                                       kwargs={'pk': self.object.id})
...
class CommentDelete(LoginRequiredMixin, generic.DeleteView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['form_url'] = reverse_lazy("mural:delete_comment", args=(),
                                           kwargs={'pk': self.object.id})
...
def suggest_users(request):
    ...
    users = User.objects.filter(Q(username__icontains=param) | Q(last_name__icontains=param) | Q(
        social_name__icontains=param)).exclude(id=request.user.id)[:5]
...
def load_comments(request, post, child_id):
    ...
    if showing == '':
        ...
        showing = showing.split(',')
        comments = Comment.objects.filter(post__id=post).exclude(id__in=showing).order_by(
            '-last_update')
