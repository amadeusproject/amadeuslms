...
class GeneralIndex(LoginRequiredMixin, LogMixin, generic.ListView):
    ...
    def get_queryset(self):
        ...
        if not favorites:
            if mines:
                general = GeneralPost.objects.extra(
                    select={
            ...
            if mines:
                ...
                ).filter(favorites_post__isnull=False, favorites_post__user=user)
        ...
        if not page:  # Don't need this if is pagination
            general_visualizations = MuralVisualizations.objects.filter(
                ...
                & Q(viewed=False)
                & (
                    Q(post__generalpost__isnull=False)
                    | Q(comment__post__generalpost__isnull=False)
                )
            ).distinct()
...
class GeneralCreate(LoginRequiredMixin, LogMixin, generic.edit.CreateView):
    ...
    def form_valid(self, form):
        ...
        notification = {
            ...
            "complete": render_to_string(
                "mural/_view.html", {"post": self.object}, self.request
            ),
            "container": ".posts",
            "accordion": False,
            "post_type": "general",
        ...
        for user in users:
            entries.append(
                MuralVisualizations(viewed=False, user=user, post=self.object)
            )
        ...
        super(GeneralCreate, self).createLog(
            self.request.user,
            self.log_component,
            self.log_action,
            self.log_resource,
    ...
    def get_success_url(self):
        return reverse_lazy(
            "mural:render_post", args=(self.object.id, "create", "gen",)
        )
...
class GeneralUpdate(LoginRequiredMixin, LogMixin, generic.UpdateView):
    ...
    def form_valid(self, form):
        ...
        super(GeneralUpdate, self).createLog(
            self.request.user,
            self.log_component,
            self.log_action,
            self.log_resource,
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context["form_url"] = reverse_lazy(
            "mural:update_general", args=(), kwargs={"pk": self.object.id}
        )
    ...
    def get_success_url(self):
        return reverse_lazy(
            "mural:render_post", args=(self.object.id, "update", "gen",)
        )
...
class GeneralDelete(LoginRequiredMixin, LogMixin, generic.DeleteView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context["form_url"] = reverse_lazy(
            "mural:delete_general", args=(), kwargs={"pk": self.object.id}
        )
    ...
    def get_success_url(self):
        ...
        super(GeneralDelete, self).createLog(
            self.request.user,
            self.log_component,
            self.log_action,
            self.log_resource,
...
def load_category_posts(request, category):
    ...
    if not favorites:
        if mines:
            posts = CategoryPost.objects.extra(
                select={
                    "most_recent": "greatest(last_update, (select max(mural_comment.last_update) from mural_comment where mural_comment.post_id = mural_categorypost.mural_ptr_id))"
                }
            ).filter(space__id=category, mural_ptr__user=user)
        else:
            posts = CategoryPost.objects.extra(
                select={
                    "most_recent": "greatest(last_update, (select max(mural_comment.last_update) from mural_comment where mural_comment.post_id = mural_categorypost.mural_ptr_id))"
                }
            ).filter(space__id=category)
    else:
        if mines:
            posts = CategoryPost.objects.extra(
                select={
    ...
    if has_page is None:
        views = MuralVisualizations.objects.filter(
            ...
            & Q(viewed=False)
            & (
                Q(comment__post__categorypost__space__id=category)
                | Q(post__categorypost__space__id=category)
            )
        )
        n_views = views.count()
...
class CategoryIndex(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        ...
        if user.is_staff:
            ...
            ).order_by("has_message", "name")
        else:
            categories = (
                Category.objects.extra(
                    select={
                        "has_message": "select 1 FROM mural_muralvisualizations AS visul LEFT JOIN mural_categorypost AS post ON visul.post_id = post.mural_ptr_id LEFT JOIN mural_comment AS com ON visul.comment_id = com.id LEFT JOIN mural_categorypost AS post2 ON com.post_id = post2.mural_ptr_id  WHERE visul.viewed = FALSE AND visul.user_id = %s AND (post.space_id = categories_category.id OR post2.space_id = categories_category.id) LIMIT 1"
                    },
                    select_params=(user.id,),
                ...
                .filter(
                    Q(coordinators__pk=user.pk)
                    | Q(subject_category__professor__pk=user.pk)
                    | Q(subject_category__students__pk=user.pk, visible=True)
                )
                .distinct()
                .order_by("has_message", "name")
            )
...
class CategoryCreate(LoginRequiredMixin, LogMixin, generic.edit.CreateView):
    ...
    def form_valid(self, form):
        ...
        simple_notify = _("%s has made a post in %s") % (
            str(self.object.user),
            str(self.object.space),
        )
        ...
        super(CategoryCreate, self).createLog(
            self.request.user,
            self.log_component,
            self.log_action,
            self.log_resource,
    ...
    def get_success_url(self):
        return reverse_lazy(
            "mural:render_post", args=(self.object.id, "create", "cat",)
        )
...
class CategoryUpdate(LoginRequiredMixin, LogMixin, generic.UpdateView):
    ...
    def form_valid(self, form):
        ...
        super(CategoryUpdate, self).createLog(
            self.request.user,
            self.log_component,
            self.log_action,
            self.log_resource,
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context["form_url"] = reverse_lazy(
            "mural:update_category", args=(), kwargs={"pk": self.object.id}
        )
    ...
    def get_success_url(self):
        return reverse_lazy(
            "mural:render_post", args=(self.object.id, "update", "cat",)
        )
...
class CategoryDelete(LoginRequiredMixin, LogMixin, generic.DeleteView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context["form_url"] = reverse_lazy(
            "mural:delete_category", args=(), kwargs={"pk": self.object.id}
        )
        context["message"] = _("Are you sure you want to delete this post?")
    ...
    def get_success_url(self):
        ...
        super(CategoryDelete, self).createLog(
            self.request.user,
            self.log_component,
            self.log_action,
            self.log_resource,
...
def load_subject_posts(request, subject):
    ...
    if has_page is None:
        views = MuralVisualizations.objects.filter(
            ...
            & Q(viewed=False)
            & (
                Q(comment__post__subjectpost__space__id=subject)
                | Q(post__subjectpost__space__id=subject)
            )
        )
        n_views = views.count()
        views.update(viewed=True, date_viewed=timezone.now())
    ...
    def get_queryset(self):
        ...
        if user.is_staff:
            ...
            subjects = (
                ...
                .filter(
                    Q(category__coordinators__pk=user.pk)
                    | Q(professor__pk=user.pk)
                    | Q(students__pk=user.pk, visible=True)
                )
                .distinct()
                .order_by("has_message", "name")
            )
...
class SubjectCreate(LoginRequiredMixin, LogMixin, generic.edit.CreateView):
    ...
    def form_valid(self, form):
        ...
        paths = [
            ...
            reverse(
                "mural:subject_view", args=(), kwargs={"slug": self.object.space.slug}
            ),
        ]
        ...
        if self.object.resource:
            paths.append(
                reverse(
                    "mural:resource_view",
                    args=(),
                    kwargs={"slug": self.object.resource.slug},
                )
            )
        ...
        super(SubjectCreate, self).createLog(
            self.request.user,
            self.log_component,
            self.log_action,
            self.log_resource,
    ...
    def get_success_url(self):
        return reverse_lazy(
            "mural:render_post", args=(self.object.id, "create", "sub",)
        )
...
class SubjectUpdate(LoginRequiredMixin, LogMixin, generic.UpdateView):
    ...
    def form_valid(self, form):
        ...
        super(SubjectUpdate, self).createLog(
            self.request.user,
            self.log_component,
            self.log_action,
            self.log_resource,
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context["form_url"] = reverse_lazy(
            "mural:update_subject", args=(), kwargs={"pk": self.object.id}
        )
    ...
    def get_success_url(self):
        return reverse_lazy(
            "mural:render_post", args=(self.object.id, "update", "sub",)
        )
...
class SubjectDelete(LoginRequiredMixin, LogMixin, generic.DeleteView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context["form_url"] = reverse_lazy(
            "mural:delete_subject", args=(), kwargs={"pk": self.object.id}
        )
        context["message"] = _("Are you sure you want to delete this post?")
    ...
    def get_success_url(self):
        ...
        super(SubjectDelete, self).createLog(
            self.request.user,
            self.log_component,
            self.log_action,
            self.log_resource,
...
class ResourceView(LoginRequiredMixin, LogMixin, generic.ListView):
        ...
        if not favorites:
            if mines:
                if not user.is_staff:
                    posts = (
                        SubjectPost.objects.extra(
                            select={
                                "most_recent": "greatest(mural_mural.last_update, (select max(mural_comment.last_update) from mural_comment where mural_comment.post_id = mural_subjectpost.mural_ptr_id))"
                            }
                        )
                        .filter(
                            Q(resource=resource)
                            & Q(mural_ptr__user=user)
                            & (
                                ...
                                | (
                                    ...
                                    & (
                                        Q(resource__all_students=True)
                                        | Q(resource__students=user)
                                        | Q(resource__groups__participants=user)
                                    )
                                )
                            )
                        )
                    ...
                    posts = SubjectPost.objects.extra(
                        select={
                            "most_recent": "greatest(mural_mural.last_update, (select max(mural_comment.last_update) from mural_comment where mural_comment.post_id = mural_subjectpost.mural_ptr_id))"
                        }
                    ).filter(resource=resource, mural_ptr__user=user)
            else:
                if not user.is_staff:
                    posts = (
                        SubjectPost.objects.extra(
            ...
            if mines:
                if not user.is_staff:
                    posts = (
                        ...
                        .filter(
                            Q(resource=resource)
                            & Q(favorites_post__isnull=False)
                            & Q(favorites_post__user=user)
                            & Q(mural_ptr__user=user)
                            & (
                                Q(space__category__coordinators=user)
                                | Q(space__professor=user)
    ...
    def get_context_data(self, **kwargs):
        ...
        if page:
            ...
            super(ResourceView, self).createLog(
                self.request.user,
                self.log_component,
                self.log_action,
                self.log_resource,
...
class ResourceCreate(LoginRequiredMixin, LogMixin, generic.edit.CreateView):
    ...
    def form_valid(self, form):
        ...
        super(ResourceCreate, self).createLog(
            self.request.user,
            self.log_component,
            self.log_action,
            self.log_resource,
...
class CommentCreate(LoginRequiredMixin, LogMixin, generic.edit.CreateView):
    ...
    def form_valid(self, form):
        ...
        if post._my_subclass == "subjectpost":
            paths.append(
                reverse(
                    "mural:subject_view",
                    args=(),
                    kwargs={"slug": post.get_space_slug()},
                )
            )
        ...
        notification = {
            ...
            "simple_notify": simple_notify,
            "complete": render_to_string(
                "mural/_view_comment.html", {"comment": self.object}, self.request
            ),
        ...
        for user in users:
            entries.append(
                MuralVisualizations(viewed=False, user=user, comment=self.object)
            )
            sendMuralPushNotification(user, self.object.user, simple_notify)
            Group("user-%s" % user.id).send({"text": notification})
        ...
        super(CommentCreate, self).createLog(
            self.request.user,
            self.log_component,
            self.log_action,
            self.log_resource,
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context["form_url"] = reverse_lazy(
            "mural:create_comment", kwargs={"post": post_id}
        )
...
class CommentUpdate(LoginRequiredMixin, LogMixin, generic.UpdateView):
    ...
    def form_valid(self, form):
        ...
        if self.object.post._my_subclass == "categorypost":
            ...
            if self.object.post.subjectpost.resource:
                self.log_context[
                    "resource_id"
                ] = self.object.post.subjectpost.resource.id
                self.log_context[
                    "resource_name"
                ] = self.object.post.subjectpost.resource.name
                self.log_context[
                    "resource_slug"
                ] = self.object.post.subjectpost.resource.slug

        super(CommentUpdate, self).createLog(
            self.request.user,
            self.log_component,
            self.log_action,
            self.log_resource,
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context["form_url"] = reverse_lazy(
            "mural:update_comment", args=(), kwargs={"pk": self.object.id}
        )
...
class CommentDelete(LoginRequiredMixin, LogMixin, generic.DeleteView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context["form_url"] = reverse_lazy(
            "mural:delete_comment", args=(), kwargs={"pk": self.object.id}
        )
    ...
    def get_success_url(self):
        ...
        super(CommentDelete, self).createLog(
            self.request.user,
            self.log_component,
            self.log_action,
            self.log_resource,
...
def suggest_users(request):
    ...
    users = User.objects.filter(
        ...
        | Q(social_name__icontains=param)
...
def load_comments(request, post, child_id):
    ...
    if showing == "":
        ...
        showing = showing.split(",")
        comments = (
            Comment.objects.filter(post__id=post)
            .exclude(id__in=showing)
