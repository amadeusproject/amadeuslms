...
class CreateView(LoginRequiredMixin, LogMixin, generic.edit.CreateView):
    ...
    def get_success_url(self):
        messages.success(
            self.request,
            _(
                'The Webpage "%s" was added to the Topic "%s" of the virtual environment "%s" successfully!'
            )
            % (
                self.object.name,
        ...
        if self.object.show_window:
            ...
            self.request.session["resources"]["new_page_url"] = reverse(
                "webpages:window_view", kwargs={"slug": self.object.slug}
            )
...
class UpdateView(LoginRequiredMixin, LogMixin, generic.UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["resource"] = get_object_or_404(
            Webpage, slug=self.kwargs.get("slug", "")
        )
    ...
    def get_success_url(self):
        messages.success(
            self.request,
            _('The Webpage "%s" was updated successfully!') % (self.object.name),
        )
...
class StatisticsView(LoginRequiredMixin, LogMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        vis_ou = Log.objects.filter(
            context__contains={"webpage_id": webpage.id},
            resource="webpage",
            action="view",
            ...
            data_history.append(
                [
                    ...
                    ", ".join(
                        [
                            str(x)
                            for x in webpage.topic.subject.group_subject.filter(
...
class SendMessage(LoginRequiredMixin, LogMixin, generic.edit.FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["webpage"] = get_object_or_404(
            Webpage, slug=self.kwargs.get("slug", "")
        )
        return context
