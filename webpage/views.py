...
class CreateView(LoginRequiredMixin, generic.edit.CreateView):
    ...
    def get_success_url(self):
        ...
        if self.object.show_window:
            ...
            self.request.session['resources']['new_page_url'] = reverse('webpages:window_view',
                                                                        kwargs={
...
class StatisticsView(LoginRequiredMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        vis_ou = Log.objects.filter(context__contains={'webpage_id': webpage.id},
                                    resource="webpage", action="view",
        ...
        for log_al in vis_ou.order_by("datetime"):
            data_history.append([str(alunos.get(email=log_al.user_email)),
                                 ", ".join([str(x) for x in
                                            webpage.topic.subject.group_subject.filter(
