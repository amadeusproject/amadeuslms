...
class CreateLinkView(LoginRequiredMixin, LogMixin, generic.edit.CreateView):
    ...
    def form_valid(self, form, pendencies_form):
        ...
        super(CreateLinkView, self).createLog(
            self.request.user,
            self.log_component,
            self.log_action,
            self.log_resource,
    ...
    def get_success_url(self):
        messages.success(
            self.request,
            _(
                'The  Link "%s" was added to the Topic "%s" of the virtual environment "%s" successfully!'
            )
            % (
                self.object.name,
...
class DeleteLinkView(LoginRequiredMixin, LogMixin, generic.edit.DeleteView):
    ...
    def get_success_url(self):
        ...
        super(DeleteLinkView, self).createLog(
            self.request.user,
            self.log_component,
            self.log_action,
            self.log_resource,
...
class RedirectUrl(LogMixin, RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        ...
        super(RedirectUrl, self).createLog(
            self.request.user,
            self.log_component,
            self.log_action,
            self.log_resource,
...
class UpdateLinkView(LoginRequiredMixin, LogMixin, generic.edit.UpdateView):
    ...
    def form_valid(self, form, pendencies_form):
        ...
        super(UpdateLinkView, self).createLog(
            self.request.user,
            self.log_component,
            self.log_action,
            self.log_resource,
    ...
    def get_success_url(self):
        messages.success(
            self.request,
            _('The Website Link "%s" was updated successfully!') % (self.object.name),
        )
...
class StatisticsView(LoginRequiredMixin, LogMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        vis_ou = Log.objects.filter(
            context__contains={"link_id": link.id},
            resource="link",
            action="view",
            ...
            data_history.append(
                [
                    ...
                    ", ".join(
                        [
                            str(x)
                            for x in link.topic.subject.group_subject.filter(
