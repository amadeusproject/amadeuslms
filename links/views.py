...
class StatisticsView(LoginRequiredMixin, LogMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        vis_ou = Log.objects.filter(context__contains={'link_id': link.id}, resource="link",
                                    action="view", user_email__in=(aluno.email for aluno in alunos),
                                    datetime__range=(
        ...
        for log_al in vis_ou.order_by("datetime"):
            data_history.append([str(alunos.get(email=log_al.user_email)),
                                 ", ".join([str(x) for x in link.topic.subject.group_subject.filter(
        ...
        for alun in not_view:
            data_n_did.append([index, str(alun), ", ".join([str(x) for x in
                                                            link.topic.subject.group_subject.filter(
