...
def most_active_users_in_a_month(request):
    ...
    mappings = {_('January'): 1, _('February'): 2, _('March'): 3, _('April'): 4, _('May'): 5,
                _('June'): 6, _('July'): 7
    ...
    data = [
        {"year": day.year, "month": day.month - 1, "day": day.day, "hour": 0, "value": day_count,
         "count": day_count} for day, day_count in data.items()]
...
def activity_in_timestamp(days, **kwargs):
    ...
    for day in days:
        if params.get('category_id'):
            ...
            day_count = Log.objects.filter(datetime__date=day, context__contains={
                "category_id": int(category_id)}).count()
        else:
...
def most_active_users(request):
    fifty_users = Log.objects.values('user_id').annotate(count=Count('user_id')).order_by('-count')[
                  :50]
...
def get_days_of_the_week_log(request):
    ...
    mapping = {0: _("Mon"), 1: _("Tue"), 2: _("Wed"), 3: _("Thu"), 4: _("Fri"), 5: _("Sat"),
               6: _("Sun")}
