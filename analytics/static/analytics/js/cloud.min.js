...
class CloudWord {
  ...
  static validData(chartConfig) {
    ...
    chartConfig.data.map(function(d) {
      ...
      chartConfig.mymax +=
          d.myvalue;  // chartConfig.mymax>d.myvalue?chartConfig.mymax:d.myvalue;
    });
  ...
  create(chartConfig) {
    ...
    if (chartConfig.max == 1) {
      ...
      this.svg.append('text')
          ...
          .attr('dy', '0em')
          .attr('class', 'NoAcess')
          .text('Esse assunto não possui tags cadastradas');
    }
    ...
    if (this.view_mode) {  // Back to cloudy
      ...
      values.selectAll('text.yours')
          ...
          .attr('transform', function(t, i) {
            return 'translate(' + [
              (w / 1.3) + w * Math.floor(i / row), h * (0.8 + (i % row)) / row
            ] + ')'
          });
      values.selectAll('text.best')
          ...
          .attr('transform', function(t, i) {
            return 'translate(' + [
              (w / 1.1) + w * Math.floor(i / row), h * (0.8 + (i % row)) / row
            ] + ')'
          });
      ...
      tbPages.on('click', (d, i) => {
        ...
        values.selectAll('text.yours').attr('transform', function(t, k) {
          return 'translate(' + [
            (w / 1.3) + w * Math.floor(k / row) - factor,
            h * (0.8 + (k % row)) / row
          ] + ')'
        });
        ...
        values.selectAll('text.best').attr('transform', function(t, k) {
          return 'translate(' + [
            (w / 1.1) + w * Math.floor(k / row) - factor,
            h * (0.8 + (k % row)) / row
          ] + ')'
        });
      });
