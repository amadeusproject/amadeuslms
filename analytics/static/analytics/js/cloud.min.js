var cloudWord = function(dataconfig,target){
    ...
    function parseText(){
        tags={};
        tags = dataconfig.data;
        generate();
    }
    function generate(){
        layout
            .font(dataconfig.font)
            .spiral(dataconfig.spiral),
        fontSize=d3v3.scale[dataconfig.scale]()//d3v3 - d3v3.scale
                    .range([10,100]),
        tags.length&&fontSize.domain([+tags[tags.length-1].value||1,+tags[0].value]),
        complete=0,
        words=[],
        layout.stop()
            .words(tags.slice(0,max=Math.min(tags.length,dataconfig.max)))
            .start()
    }
    function draw(t,e){
        globala.push(t);
        w=dataconfig.dimensions.w;
        h=dataconfig.dimensions.h;
        layout.size([w,h]);
        ...
        d3.validEvents(dataconfig.interactions);
        if(dataconfig.tooltip!=undefined){
            var toolTip = new ToolTip(dataconfig.tooltip);
            dataconfig.interactions.mouseover.push(function (element, data) {
                toolTip.show(data);
            });
            dataconfig.interactions.mousemove.push(function (element, data) {
                toolTip.move();
            });
            dataconfig.interactions.mouseout.push(function (element, data) {
                toolTip.hide();
            });
        }
        d3.addEvents(n,dataconfig.interactions);
        
        
        var a=background.append("g").attr("style","position: relative; font-family: \"Helvetica Neue\", sans-serif; width: 960px; margin: auto; margin-bottom: 1em; margin-top: 20px;")
    ...
    (function(dataconfig){
        if(dataconfig.data == undefined || dataconfig.data[0]["key"]==undefined || dataconfig.data[0]["value"]==undefined)
            console.error("Dataset invalid!");
        if(dataconfig.max == undefined)dataconfig.max = 50;
        if(dataconfig.font == undefined)dataconfig.font = "Impact";
        if(dataconfig.spiral == undefined)dataconfig.spiral="rectangular";// archimedean | rectangular
        if(dataconfig.scale == undefined)dataconfig.scale="log";// linear | sqrt | log
        if(dataconfig.angles == undefined)dataconfig.angles = {};
            if(dataconfig.angles.from == undefined)dataconfig.angles.from = 0;
            if(dataconfig.angles.to == undefined)dataconfig.angles.to = 0;
            if(dataconfig.angles.n == undefined)dataconfig.angles.n = 1;
        if(dataconfig.dimensions == undefined)dataconfig.dimensions = {};
            if(dataconfig.dimensions.w == undefined)dataconfig.dimensions.w = 800;
            if(dataconfig.dimensions.h == undefined)dataconfig.dimensions.h = 400;
        if(dataconfig.interactions == undefined)dataconfig.interactions = {};
        if(dataconfig.tooltip != undefined && (dataconfig.tooltip.text == undefined || dataconfig.tooltip.text == ""))
            dataconfig.tooltip = undefined;
        else{
            if(dataconfig.tooltip.name == undefined)dataconfig.tooltip.name = "Wordcloud_tooltip";
            if(dataconfig.tooltip.parent == undefined)dataconfig.tooltip.parent = "#vis_svg";
        }
    })(dataconfig);

    dataconfig.data.sort(function(d1,d2){return d1.value<d2.value?1:(d1.value>d2.value?-1:0)})
    dataconfig.data = dataconfig.data.slice(0,dataconfig.max);
    ...
    var fill=d3.scaleCategory(),//d3v3.schemeCategory10,//d3v3.scale.category20b(),
        ...
        layout.on("end",draw),
        svg=d3v3.select(target).append("svg").attr("width",w).attr("height",h).attr("id","vis_svg"),
        background=svg.append("g"),
        vis=svg.append("g").attr("transform","translate("+[w>>1,h>>1]+")");
            
        (function(){
            function t(){
                c=+dataconfig.angles.n,
                u=Math.max(-90,Math.min(90,+dataconfig.angles.from)),
                i=Math.max(-90,Math.min(90,+dataconfig.angles.to)),
                e()
            }
            function e(){
