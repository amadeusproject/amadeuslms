...
class CloudWord {
    ...
    view_toogle() {
        ...
        if (this.view_mode) {
            ...
            var qtd = a.dataUsed.length;
            var h = document.querySelector("#vis_svg").getBoundingClientRect();
            var w = h.width;
            h = h.height;
            ...
            d3.select("#cloudy-icon").attr("class","fas fa-sort-numeric-up-alt");
            svg.select(".vis")
                .transition().duration(1e3)
                .attr("transform", "translate(0,0)")
                abrev_init();
            svg.select(".vis").selectAll("text").text(function(d){return abreviate_ifneed("",d.key,w/col-2*h/row,0.8 * h / row) })
            ...
            svg.select(".vis").selectAll("text")
                ...
                .attr("transform", function (t, i) {
                    return "translate(" + [h/(2*row) + w / (2*col) + w / col *  Math.floor(i / row),
                    h * (0.8 + (i % row)) / row] + ")"
                })
                .attr("fill","#000000")
            background.attr("opacity",0).attr("transform","scale(1,1)");
            background.selectAll(".rows-table").data(this.dataUsed).enter().append("g").attr("class","rows-table");
            background.selectAll(".rows-table").data(this.dataUsed).exit().remove();
            var rowsback = background.selectAll(".rows-table").data(this.dataUsed)
