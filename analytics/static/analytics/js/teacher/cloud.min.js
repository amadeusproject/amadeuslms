...
class CloudWord {
  constructor(chartConfig) {
    ...
    if (this.chartConfig.default) this.chartConfig.default();
  }
  static validData(chartConfig) {
    if (chartConfig.data == undefined || chartConfig.data[0] == undefined ||
        chartConfig.data[0]['key'] == undefined ||
        chartConfig.data[0]['value'] == undefined)
      console.error('Dataset invalid!');

    if (chartConfig.max == undefined) chartConfig.max = 30;
    if (chartConfig.font == undefined) chartConfig.font = 'Impact';
    if (chartConfig.spiral == undefined)
      chartConfig.spiral = 'archimedean';  // archimedean | rectangular
    if (chartConfig.scale == undefined)
      chartConfig.scale = 'log';  // linear | sqrt | log
    ...
    if (chartConfig.tooltip != undefined &&
        (chartConfig.tooltip.text == undefined ||
         chartConfig.tooltip.text == ''))
      chartConfig.tooltip = undefined;
    else {
      if (chartConfig.tooltip.name == undefined)
        chartConfig.tooltip.name = 'Wordcloud_tooltip';
      if (chartConfig.tooltip.parent == undefined)
        chartConfig.tooltip.parent = '#vis_svg';
    }
    if (chartConfig.filltext == undefined) {
      chartConfig.filltext = function(a) {
    ...
    chartConfig.data.map(function(d) {
      chartConfig.max +=
          d.value;  // chartConfig.max>d.value?chartConfig.max:d.value;
      if (chartConfig.max > 0) {
        chartConfig.max = 30;
      }
    });
    if (chartConfig.max == 0 || isNaN(chartConfig.max)) chartConfig.max = 1;



    chartConfig.data = chartConfig.data.filter(function(d) {
      return d.value > 0
    })
  ...
  create(chartConfig) {
    ...
    this.svg = d3v3.select(a.chartConfig.parent)
                   .append('svg')
                   .attr('width', a.chartConfig.dimensions.w)
                   .attr('height', a.chartConfig.dimensions.h)
                   .attr('id', 'vis_svg')
    if (chartConfig.max == 1) {
  ...
  posdraw() {
    ...
    // var
    // fill=d3.scaleCategory(),//d3v3.schemeCategory10,//d3v3.scale.category20b(),
  ...
  draw() {
    ...
    function generate() {
      layout.font(j.chartConfig.font).spiral(j.chartConfig.spiral),
          ...
          layout.stop()
              .words(
                  tags.slice(0, max = Math.min(tags.length, j.chartConfig.max)))
              .start()
    }
    function draw(t, e) {
      ...
      scale = e ?
          Math.min(
              w / Math.abs(e[1].x - w / 2), w / Math.abs(e[0].x - w / 2),
              h / Math.abs(e[1].y - h / 2), h / Math.abs(e[0].y - h / 2)) /
              2.2 :
          1,
      words = t;
      j.dataUsed = words;
      ...
      var a = background.selectAll('.textgroup')
                  .data([1])
                  .enter()
                  .append('g')
                  .attr('class', 'textgroup')
      a = background.selectAll('.textgroup')
              .data([1])
              .attr(
                  'style',
                  'position: relative; font-family: "Helvetica Neue", sans-serif; width: 960px; margin: auto; margin-bottom: 1em; margin-top: 20px;')
    ...
    !function(t) {
      function e() {
        function t(t, n, a) {
          for (var r, o, s, l = ([{x: 0, y: 0}, {x: e[0], y: e[1]}], n.x),
                            i = n.y, d = Math.sqrt(e[0] * e[0] + e[1] * e[1]),
                            h = m(e), f = Math.random() < .5 ? 1 : -1, p = -f;
               (r = h(p += f)) &&
               (o = ~~r[0], s = ~~r[1], !(Math.min(o, s) > d));)
            if (n.x = l + o, n.y = i + s,
                !(n.x + n.x0 < 0 || n.y + n.y0 < 0 || n.x + n.x1 > e[0] ||
                  n.y + n.y1 > e[1]) &&
                    (!a || !u(n, t, e[0])) && (!a || c(n, a))) {
              for (var y, g = n.sprite, v = n.width >> 5, x = e[0] >> 5,
                          w = n.x - (v << 4), M = 127 & w, b = 32 - M,
                          z = n.y1 - n.y0, k = (n.y + n.y0) * x + (w >> 5),
                          T = 0;
                   z > T; T++) {
                y = 0;
                for (var A = 0; v >= A; A++)
        ...
        var e = [256, 256], h = n, p = a, y = r, g = o, v = s, m = d, x = [],
            w = 1 / 0, b = d3v3.dispatch('word', 'end'), z = null, k = {};
        return k.start = function() {
          function n() {
            for (var n, s = +new Date; + new Date - s < w && ++u < o && z;)
              n = d[u], n.x = e[0] * (Math.random() + 0.5) >> 1,
          ...
          var a = f((e[0] >> 5) * e[1]), r = null, o = x.length, u = -1, c = [],
      ...
      function l(t, e, n) {
        if (!t.sprite) {
          ...
          for (n--; ++n < s;) {
            ...
            if (t.rotate) {
              var i = Math.sin(t.rotate * y), c = Math.cos(t.rotate * y),
                  d = l * c, h = l * i, f = u * c, p = u * i;
              l = Math.max(Math.abs(d + p), Math.abs(d - p)) + 31 >> 5 << 5,
              u = ~~Math.max(Math.abs(h + f), Math.abs(h - f))
            } else
              l = l + 31 >> 5 << 5;
            if (u > o && (o = u), a + l >= g << 5 && (a = 0, r += o, o = 0),
                r + u >= v)
              break;
            w.translate((a + (l >> 1)) / m, (r + (u >> 1)) / m),
                t.rotate && w.rotate(t.rotate * y), w.fillText(t.text, 0, 0),
                w.restore(), t.width = l, t.height = u, t.xoff = a, t.yoff = r,
                             t.x1 = l >> 1, t.y1 = u >> 1, t.x0 = -t.x1,
                             t.y0 = -t.y1, a += l
          }
          for (var x = w.getImageData(0, 0, (g << 5) / m, v / m).data, M = [];
               --n >= 0;) {
            t = e[n];
            for (var l = t.width, b = l >> 5, u = t.y1 - t.y0, z = t.padding,
                     k = 0;
                 u * b > k; k++)
              M[k] = 0;
      ...
      function u(t, e, n) {
        ...
        for (var a, r = t.sprite, o = t.width >> 5, s = t.x - (o << 4),
                    l = 127 & s, u = 32 - l, i = t.y1 - t.y0,
                    c = (t.y + t.y0) * n + (s >> 5), d = 0;
             i > d; d++) {
          a = 0;
          for (var h = 0; o >= h; h++)
      ...
      function i(t, e) {
        var n = t[0], a = t[1];
        e.x +
                e.x0<n.x && (n.x = e.x + e.x0),
                     e.y + e.y0<n.y && (n.y = e.y + e.y0), e.x + e.x1>a.x &&
                         (a.x = e.x + e.x1),
                     e.y + e.y1>a.y &&
            (a.y = e.y + e.y1)
      }
      function c(t, e) {
        return t.x + t.x1 > e[0].x && t.x + t.x0 < e[1].x &&
            t.y + t.y1 > e[0].y && t.y + t.y0 < e[1].y
      }
      function d(t) {
        var e = t[0] / t[1];
        return function(t) {
      ...
      if ('undefined' != typeof document)
        p = document.createElement('canvas'), p.width = 1, p.height = 1,
        m = Math.sqrt(
            p.getContext('2d').getImageData(0, 0, 1, 1).data.length >> 2),
        p.width = (g << 5) / m, p.height = v / m;
      else {
    ...
    }('undefined' == typeof exports ? d3v3.layout || (d3v3.layout = {}) :
                                      exports);

    j.chartConfig.data.sort(function(d1, d2) {
      return d1.value < d2.value ? 1 : (d1.value > d2.value ? -1 : 0)
    })
    j.chartConfig.data = j.chartConfig.data.slice(0, j.chartConfig.max);
    // var svg;
    var w = j.chartConfig.dimensions.w, h = j.chartConfig.dimensions.h,
        words = [], max, scale = 1, complete = 0, tags, fontSize,
        layout = d3v3.layout.cloud()
                     ...
                     .on('end', draw),
        svg = j.svg,
        background = svg.selectAll('.background')
                         .data([1])
                         .enter()
                         .append('g')
                         .attr('class', 'background'),
        vis = svg.selectAll('.vis').data([1]).enter().append('g').attr(
            'class', 'vis'),
        values = svg.selectAll('.values').data([1]).enter().append('g').attr(
            'class', 'values'),
        pagination = svg.selectAll('.pagination')
                         .data([1])
                         .enter()
                         .append('g')
                         .attr('class', 'pagination');
    background = svg.selectAll('.background').data([1]),
    vis = svg.selectAll('.vis').data([1]).attr(
        'transform', 'translate(' + [w >> 1, h >> 1] + ')');
    values = svg.selectAll('.values').data([1]).attr(
        'transform', 'translate(' + [w >> 1, h >> 1] + ')');
    ...
    (function() {
      function t() {
        ...
        i = Math.max(-90, Math.min(90, +j.chartConfig.angles.to)), e()
      }
      function e() {
        h.domain([0, c - 1]).range([u, i]);

        var t = l.selectAll('path.angle').data([
          {startAngle: u * d, endAngle: i * d}
        ]);
        ...
        var o = l.selectAll('line.angle').data(d3v3.range(c).map(h));

        o.enter().append('line').attr('class', 'angle'), o.exit().remove(),
        ...
        s.enter()
            ...
            .call(
                d3v3.behavior
                    .drag()  // d3v3.drag() - d3v3.behavior.drag()
                    .on('drag',
                        function(t, o) {
                          t = (o ? i : u) + 90;
            ...
            d3v3.select('#angle-from').property('value', u),
            d3v3.select('#angle-to').property('value', i)
      }
      function n(t, e) {
      ...
      l.append('path')
          ...
          l.selectAll('text')
              ...
              .attr(
                  ...
                  function(t) {
                    return t += 90,
                           'rotate(' + t + ')translate(' + -(r + 10) +
                               ')rotate(' + -t + ')translate(2)'
                  })
              .text(function(t) {
                return t + '\xb0'
              });
      ...
      var u, i, c,
          d = Math.PI / 180, h = d3v3.scale.linear(),
          f = d3v3.svg.arc().innerRadius(0).outerRadius(
              r);  // scaleLinear - scale.linear//d3v3.arc - d3v3.svg.arc
  ...
  view_toogle() {
    ...
    if (this.view_mode) {  // Back to cloudy
      ...
      d3.select('#cloudy-icon')
          .attr('class', 'fa fa-text-height icon-cloudy-legend');

      $('#tableHeader').fadeOut();

      svg.select('.vis').selectAll('.no_cloud').remove();

      svg.select('.vis').transition().duration(1e3).delay(300).attr(
          'transform', 'translate(' + [
            a.chartConfig.dimensions.w / 2, a.chartConfig.dimensions.h / 2
          ] + ')')
      svg.select('.vis')
          .selectAll('text')
          .data(this.dataUsed)
          ...
          .attr(
              'fill',
              function(d) {
                return a.fillpattern(a.scalePercent(d.value, 1));
              })
          .attr('text-anchor', 'middle')
          .text(function(d) {
            return d.key
      ...
      svg.selectAll('.NoAcess').style('display', 'block')
    } else {
      var qtd = 10;
      ...
      d3.select('#cloudy-icon')
          .attr('class', 'fa fa-sort-amount-desc icon-cloudy-legend');

      $('#tableHeader').css('display', 'flex');

      if (this.chartConfig.max == 1) {
        this.svg.selectAll('.NoAcess').style('display', 'none')
      }
      ...
      svg.select('.vis')
          ...
          .attr(
              ...
              function(t, i) {
                return 'translate(' + [
                  h / (0.9 * row) + w / col * Math.floor(i / row),
                  h * (0.8 + (i % row)) / row
                ] + ')'
              })
      ...
      values.selectAll('text.best')
          ...
          .attr('transform', function(t, i) {
            return 'translate(' + [
              (w / 1.18) + w * Math.floor(i / row), h * (0.8 + (i % row)) / row
            ] + ')'
          });
      ...
      d3.addEvents(
          svg.select('.vis').selectAll('text').data(missedTags),
          a.chartConfig.interactions);

      svg.select('.vis').transition().duration(1e3).attr(
      ...
      svg.select('.vis').selectAll('text').text(function(d) {
        return abreviate(d.key, w / col - 2 * h / row, 0.8 * h / row)
      });
      ...
      background.selectAll('.rows-table')
          .data(this.chartConfig.tableData)
          .enter()
          .append('g')
          .attr('class', 'rows-table');
      background.selectAll('.rows-table')
          .data(this.chartConfig.tableData)
          .exit()
          .remove();
      var rowsback =
          background.selectAll('.rows-table')
              ...
              .attr('transform', function(d, i) {
                return 'translate(' + [
                  w / col * Math.floor(i / row) + 1, h * (i % row) / row + 1
                ] + ')'
              });
      var rects = [
      ...
      var texts = [
        {
          ...
          text_anchor: 'middle',
          font_size: 0.6 * h / row,
          transform: 'translate(' + [h / (2 * row), h / (2 * row)] + ')',
          dy: '0.3em',
          text: '.'
        },
      ];

      rowsback.selectAll('.innerRect')
          .data(rects)
          .enter()
          .append('rect')
      ...
      rowsback.selectAll('.innerText')
          ...
          .attr(
              'text-anchor',
              function(d) {
                if (d.text_anchor) return d.text_anchor
              })
          .attr(
              'font-size',
              function(d) {
                if (d.font_size) return d.font_size
              })
          .attr(
              'transform',
              function(d) {
                if (d.transform) return d.transform
              })
          .attr(
              'dy',
              function(d) {
      ...
      const tbPages =
          svg.select('.pagination')
              ...
              .data([...Array(pages).keys()])
              .attr(
                  'transform', (d, i) => `translate(${1 + i * 30}, ${h + 10})`);

      tbPages.selectAll('.pageBtn')
          .data([1])
          .enter()
          .append('rect')
          .attr('class', 'pageBtn')
      ...
      tbPages.selectAll('text').data([1]).enter().append('text').attr(
          'transform', (d, i) => `translate(12, 20)`);
      ...
      tbPages.on('click', (d, i) => {
        ...
        background.selectAll('.rows-table')
            .data(a.chartConfig.tableData)
            .attr('transform', function(t, k) {
              return 'translate(' + [
                (w / col * Math.floor(k / row) + 1) - factor,
                h * (k % row) / row + 1
              ] + ')'
            });
        ...
        svg.select('.vis').selectAll('text').attr('transform', function(t, k) {
          return 'translate(' + [
            (h / (0.9 * row) + w / col * Math.floor(k / row)) - factor,
            h * (0.8 + (k % row)) / row
          ] + ')'
        });
        ...
        values.selectAll('text.best').attr('transform', function(t, k) {
          return 'translate(' + [
            (w / 1.18) + w * Math.floor(k / row) - factor,
            h * (0.8 + (k % row)) / row
          ] + ')'
        });
      });
