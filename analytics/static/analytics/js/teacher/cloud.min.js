...
class CloudWord {
    ...
    static validData(chartConfig) {
        if (chartConfig.data == undefined || chartConfig.data[0] == undefined || chartConfig.data[0]["key"] == undefined || chartConfig.data[0]["value"] == undefined)
            console.error("Dataset invalid!");
        if (chartConfig.max == undefined) chartConfig.max = 20;
    ...
    draw() {
        ...
        function draw(t, e) {
            ...
            scale = e ?
                Math.min(w / Math.abs(e[1].x - w / 2), w / Math.abs(e[0].x - w / 2), h / Math.abs(e[1].y - h / 2), h / Math.abs(e[0].y - h / 2)) / 2.2
                : 1,
                words = t;
            j.dataUsed = words;
    ...
    view_toogle() {
        ...
        if (this.view_mode) { //Back to cloudy
            ...
            values.selectAll("text.best").data(tableData).enter().append("text").attr("class", 'best').attr("transform", function (t, i) {
              return "translate(" + [(w / 1.18) + w *  Math.floor(i / row),
              h * (0.8 + (i % row)) / row] + ")"
          });
            ...
            tbPages.on("click", (d, i) => {
              ...
              values.selectAll("text.best").attr("transform", function (t, k) {
                return "translate(" + [(w / 1.18) + w * Math.floor(k / row) - factor,
                  h * (0.8 + (k % row)) / row] + ")"
              });
            });
        }
