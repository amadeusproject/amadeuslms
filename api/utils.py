...
def send_chat_push_notification(user, message):
    ...
    if device is not None:
        ...
        device.send_message(title=str(message.user), body=simple_notify,
                            data={"response": response, "title": title, "body": simple_notify,
                                  "user_from": message.user.email, "user_name": str(message.user),
                                  "user_img": message.user.image_url, "type": 'chat',
...
def sendMuralPushNotification(user, user_action, message):
    ...
    if not device is None:
        device.send_message(
            data={"title": "Mural", "body": message, "user_img": user_action.image_url,
                  "type": "mural"})
