            ...
            if user is not None:
                if not security.maintence or user.is_staff:
                    if oauth.count() > 0:
                        ...
                        uri = request.build_absolute_uri(reverse('oauth2_provider:token')).replace(
                            'http', 'https')
...
class LoginViewset(viewsets.ReadOnlyModelViewSet):
    ...
    def login(self, request):
        ...
        if not user is None:
            ...
            user_info = {"data": json_r, "message": "", "type": "", "title": "", "success": True,
                         "number": 1, 'extra': 0}
    ...
    def register_device(self, request):
        ...
        if not user is None:
            ...
            if not fcm_d.pk is None:
                FCMDevice.objects.filter(registration_id=device).exclude(pk=fcm_d.pk).update(
                    active=False)
    ...
    def get_subjects(self, request):
        ...
        if not user is None:
            if user.is_staff:
                ...
                subjects = Subject.objects.filter(Q(students__pk=pk) | Q(professor__pk=pk) | Q(
                    category__coordinators__pk=pk)).distinct()
    ...
    def get_participants(self, request):
        ...
        if not subject_slug == "":
            ...
            participants = User.objects.filter(
                Q(is_staff=True) | Q(subject_student__slug=subject_slug) | Q(
                    professors__slug=subject_slug) | Q(
                    coordinators__subject_category__slug=subject_slug)).exclude(
                email=username).distinct()
...
class ChatViewset(viewsets.ModelViewSet, ):
        ...
        if not user_two == "":
            ...
            messages = TalkMessages.objects.filter(
                (Q(talk__user_one__email=username) & Q(talk__user_two__email=user_two)) | (
                        Q(talk__user_one__email=user_two) & Q(
                    talk__user_two__email=username))).order_by('-create_date')

            views = ChatVisualizations.objects.filter(Q(user=user) & (
                    Q(message__talk__user_two__email=user_two) | Q(
                message__talk__user_one__email=user_two)) & Q(viewed=False))
        ...
        if not user_two == "" and not username == "":
            ...
            if not message.pk is None:
                ...
                notification = {
                    ...
                    "complete": render_to_string("chat/_message.html", {"talk_msg": message},
                                                 request),
                    "container": "chat-" + str(message.user.id),
                    "last_date": _("Last message in %s") % (
                        formats.date_format(message.create_date, "SHORT_DATETIME_FORMAT"))
                }

                notification = json.dumps(notification)

                channel_layer = get_channel_layer()
                async_to_sync(channel_layer.send)("user-%s" % user_to.id, {'text': notification})
    ...
    def favorite_messages(self, request):
        ...
        for i in range(0, list_size):
            ...
            if favor == "true":
                if not ChatFavorites.objects.filter(
                        Q(user=user) & Q(message__id=message_id)).exists():
        ...
        info = {"message": "", "type": "", "title": "", "success": True, "number": 1, 'extra': 0}
...
class MuralViewset(viewsets.ModelViewSet, ):
    ...
    def get_posts(self, request):
        ...
        if n_page is None or n_page == 0:
            views = MuralVisualizations.objects.filter(Q(user=user) & Q(viewed=False) & (
                    Q(comment__post__subjectpost__space__id=sub.id) | Q(
                post__subjectpost__space__id=sub.id)))
            views.update(viewed=True, date_viewed=datetime.now())
        ...
        serializer = MuralSerializer(page, many=True,
                                     context={"request_user": user, "subject": subject})
    ...
    def get_comments(self, request):
        ...
        serializer = CommentsSerializer(page, many=True, context={"request_user": mural.user,
    ...
    def create_comment(self, request):
        ...
        if not comment.pk is None:
            ...
            notification = {
                ...
                "user_icon": user.image_url,
                "simple_notify": simple_notify,
                "complete": render_to_string("mural/_view_comment.html", {"comment": comment},
                                             request),
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if username is not None and subject is not None:
                notifications = Notification.objects.filter(user__email=username,
                                                            task__resource__topic__subject__slug=subject).annotate(
                    str_date=Cast('creation_date', TextField())).values('str_date').order_by(
                    '-str_date').annotate(total=Count('str_date'))
